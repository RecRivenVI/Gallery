...
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携测试基线与同源切换主路径） | 🟡（本地制品及切换/恢复 smoke 通过，正式 Gate 未通过） | 精确干净提交可生成同源双前端 ZIP、三份 SBOM、清单、摘要与签名状态门禁；两个独立版本标签已在同 AppDirs 完成用户事实承接、备份校验与重启恢复 | 正式签名、安装/更新、真实历史升级/降级与失败回滚、平台矩阵及桌面壳未完成 | 完成 Windows RC 门禁 |
