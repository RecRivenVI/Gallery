...
- SourceWork 消失但 Source 仍在线：WorkBinding 转为 inactive，CanonicalWork 与 Overlay 保留；同一来源稳定键重现时复用原 Canonical 实体并恢复 active。`orphaned` 保留给显式删除确认或保留窗口之后的长期消失，不由单次未发现自动产生。
