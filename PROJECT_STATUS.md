...
| 阶段 6 业务闭环 | 「普通浏览器完成所有业务闭环后，才开始壳集成」 | **完成度被高估后已下调，现恢复首批持续切片** | EV-39 实测发现 WebSocket 与 capability 阻断；EV-40 修复，EV-54～EV-60 又以隔离真实后端覆盖管理自举、扫描 publication、WS→HTTP snapshot、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全管理与断线恢复 | EV-39/40 真实 Chrome/Edge 探针；EV-54～EV-60 Chromium/真实 `galleryd` 合成 Source 持续 E2E | 已覆盖规则与安全管理首链，但任意规则的完整可视化构建及完整业务闭环条件仍未满足，不得进入壳集成 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-60 | 🟡 | Vitest 14 个文件 177 项；Chromium mock smoke 4/4；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium/真实 `galleryd` 为 Personal 8/8、LAN 1/1 与 Source guard | 管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全资源管理与断线恢复已纳入 CI workflow；任意规则完整可视化、治理/维护/任务操作等写路径、Firefox/真实移动设备/屏幕阅读器仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-60 又覆盖管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全管理和断线 snapshot 恢复。但 Firefox、真实移动设备/屏幕阅读器、窄屏侧栏焦点陷阱、任意规则的完整可视化构建、治理/维护/任务操作等写路径 E2E 与正式可用性 Gate 均未完成 |
| 测试体系缺口 | EV-54～EV-60 已让管理自举、画廊/媒体、CustomCover、规则 lifecycle/ParameterSet/Schema 表单、安全资源管理与断线恢复的 Chromium/真实后端 E2E 进入 CI workflow，并保持 mock smoke 与真实证据分层；EV-45 已让阶段 4 testlab Correctness 进入普通 `go test`，但 500k 性能/十来源矩阵仍是人工门禁；全仓库仍无正式 Fuzz，部分平台包仍缺直接测试。显式 WS sequence gap/弱网抖动、任意规则完整可视化和治理/维护/任务操作等路径仍未进入真实后端持续门禁 |
...
| 3 | 阶段 6 | 在 EV-54～EV-60 的管理自举、画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全管理与断线恢复门禁上，继续覆盖任意规则的完整可视化构建、治理/维护/任务操作等写路径、显式 WS sequence gap/弱网抖动、浏览器/可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
