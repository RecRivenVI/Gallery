...
- **无法安全自动处理时复用 Binding issue 体系**：产生 `SOURCE_WORK_SPLIT_REVIEW_REQUIRED` 或 `SOURCE_WORK_MERGE_REVIEW_REQUIRED` 审查记录并默认阻塞该 Source publication。issue 携带原 SourceWork 稳定键、原 CanonicalWork、新 SourceWork 候选、证据指纹与结构类别（`structure_kind`）；相同证据不重复产生，证据变化 superseded。
...
- **决策可撤销**：决策与其 pre-seed Binding 溯源持久化在 `source_structure_decisions`。尚未被扫描消费（新 source_key 未产生 active Binding）时撤销可清除 pre-seed Binding 并重开 issue；已被消费（已产生新 Binding，可能已有 Overlay/Favorite/Progress 依赖）时返回结构化 `CONFLICT`，不做不可靠逆向重建。
