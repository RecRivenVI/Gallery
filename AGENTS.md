...
- 根目录 `README.md` 和 `开发进度.md` 是面向普通用户和潜在贡献者的项目介绍与阶段摘要，用于快速了解现状，不是规范、ADR、实施计划或验证记录的替代品，不承载它们的权威语义；两者的定位、触发同步的情形和更新方式见「文档维护」一节。
...
### `README.md` 与 `开发进度.md` 的定位与同步

- 根目录 `README.md` 是面向普通用户和潜在贡献者的项目首页：产品定位、特色功能、技术栈、当前可用性提示和总体进度摘要。
- 根目录 `开发进度.md` 是面向一般读者的完整进度、测试门禁、限制和后续路线汇总，包含逐阶段的细粒度功能状态与测试状态。
- 两者都是**用户向摘要**，不是规范、ADR、实施计划或验证记录的替代品：功能实现结论以事实代码为准，测试或门禁结论以 `Documents/证据/验证记录.md` 为准。若这两份文档与权威规范、实施计划、ADR 或验证记录冲突，应修正这两份用户向文档，不得让它们覆盖或重新定义权威来源。
- README 中的总体进度表是 `开发进度.md` 总体进度表的简明副本：阶段状态、Emoji、最大缺口和下一步必须与 `开发进度.md` 保持一致；README 可以更简洁，但不得对完成度作更乐观的表述。`开发进度.md` 必须保留更细的逐阶段功能与测试双状态表格。README 中列出的特色功能必须能在事实代码和 `开发进度.md` 中找到依据；在尚无 UI、安装包或正式发行版本之前，不得写出"可安装""可日常使用"等使用就绪的措辞。

#### 触发同步的情形

每次完成下列任一大型开发后，必须检查并同步更新 `README.md` 与 `开发进度.md`：

- 一个编号阶段或命名切片（Walking Skeleton、Architecture Proof）完成；
- Correctness 收口；
- Freeze Gate、Reference Performance Gate、Degradation Gate 或平台门禁状态变化；
- 新增或完成跨多个模块的正式能力；
- 公开 API、查询、媒体、规则、任务、数据所有权或恢复语义发生显著变化；
- 新增用户可感知的特色能力；
- 某项原本未完成、半成品、PRE_FREEZE 或延期的状态发生变化；
- `Documents/证据/验证记录.md` 增加了会改变对完成度理解的证据，或旧验证结论被新验证推翻或修正；
- 项目进入新的主要阶段；
- Web/PWA、账户安全、平台适配或发行状态发生重要变化。

拼写修正、内部重构或不影响对外状态的局部 bugfix 不强制要求修改这两份用户文档，但仍须判断该改动是否会改变现有描述；即使最终判断无需改动，也应在任务过程中明确说明已经检查过这两份文档，不得默默跳过判断。

#### 更新方式：禁止打补丁式追加

大型开发完成后的用户文档同步不得采用追加补丁式维护。Agent 必须重新审视并重写 `README.md` 与 `开发进度.md` 中所有受影响的摘要、章节、表格行、状态、限制和后续计划，删除或修正已经失效的表述。不得仅在文档末尾追加日期说明、新 EV 链接或"现已完成"注释来掩盖正文仍然过时的问题。具体禁止：

- 只在 README 或 `开发进度.md` 末尾追加一条日期记录；
- 保留旧状态表，再在下方补一句"现已完成"；
- 用新的注释覆盖仍然存在的过时正文；
- 只增加一个新 EV 链接，而不修正被该 EV 推翻的旧结论；
- 复制新的阶段摘要而让旧摘要并存；
- 让 README 总体表和 `开发进度.md` 总体表出现不同状态。

正确做法：

1. 找到所有受变化影响的章节、表格行、摘要和链接；
2. 重新编写这些区域；
3. 删除或改写已经过时的表述；
4. 合并重复信息；
5. 同步更新两个文件中有意重复的总体进度表；
6. 保留历史演进时，明确写出旧结论被何时、因何证据修正；
7. 使文档在不阅读末尾追加说明的情况下，也能直接得到当前正确结论。
...
本词表只定义提交 `scope` 标签，不改写产品和文档的正式术语或分类。`目录` 用作 Catalog 领域的 `scope` 时，正文仍使用正式术语 Catalog；`指南` 汇总实施计划、测试门禁和验证记录的提交作用域，不改变验证记录在文档导航中的“证据”分类。
