...
- 当前仓库已有正式产品代码（`cmd/`、`internal/`、`pkg/`、`web/`）。阶段 0 契约骨架、Walking Skeleton、Architecture Proof、阶段 1「领域和数据所有权」、阶段 2「规则闭环」、阶段 3「扫描、任务和 Catalog」与阶段 4「查询和媒体」均已完成代码与合成 Correctness；阶段 5 已完成账户/安全/多客户端代码与合成收尾，并取得同机 Personal/LAN 与当前工作站 Argon2id 补证，但正式 Security Gate 未通过；阶段 6 已实现 React/TypeScript Web/PWA 双入口及认证、浏览/媒体、Overlay、任务、规则、安全和维护页面，EV-54～EV-91 已将主要业务、治理、恢复、弱网切片、响应式、Source 作者与授权 Job 历史接入 Chromium/Firefox 真实 `galleryd` 门禁，但正式 Web Gate 未通过。EV-92 首次把 legacy schema v3 的真实 Pixiv 规则经正式转换接入独立 AppDirs，在 370,712 文件/105,202 目录上完成 45 秒有界 `index` 与全树零写入 guard；EV-93 修复 discovery 不响应 Scan context，EV-94 再以真实 Pixiv 将扫描总耗时从修复前 116,584 ms 收敛到 45,397 ms，并在取消 POST 后 201 ms 观察到终态、同 AppDirs 恢复重跑 7/7 通过。该结论只覆盖 Windows 本地 SSD/Pixiv/index discovery，不是全量扫描/哈希、完整规则语义或性能 Gate。阶段 4 的 EV-35/36/51/52/87～89 已建立 500,000 合成 Correctness 与多项候选/聚合性能基线，EV-95 又关闭 Creator 治理入口的无界响应；EV-96 已建立十来源、全局 1%/10%/50% 变化的生产 Store publication 执行器并通过 1,000 Work 预检，但正式 500,000 多样本、并发、冷缓存、Degradation 与 API Freeze 仍未完成；Gank/Pawchive 有界验证也仍未完成。
...
- [EV-96](Documents/证据/验证记录.md)（2026-07-29）增加独立 `publication-perf` 执行器：十 Source 以主 Source 占全局 50% 的加权语料，精确执行全局 1%/10%/50% 变化并沿生产 Store 完成 Stage/Overlay/Validate/Publish/GC/Checkpoint；每轮核对完整关系、媒体、Blob、Location、FTS、稳定 Canonical 身份、历史 publication、阶段空间和原子报告。`reference` 模式强制 500,000 Work 且每档至少 20 样本，降级形态不能冒充正式结果。当前仅完成 1,000 Work、每档 2 样本的 6/6 预检，正式规模、并发、冷缓存和 Degradation 仍未执行，Reference/API Freeze Gate 不变。
...
- 尚未完成：阶段 5 真实物理 LAN 多设备、目标低端设备 Argon2id 和真实恶意资源门禁；阶段 6 真实移动设备/触控、人工屏幕阅读器、任意规则的完整可视化构建、真实存储上的取消与崩溃恢复响应、完整弱网抖动矩阵与完整可访问性 E2E；以及 Pixiv/真实 HDD 全量扫描与哈希、SMB/NAS、真实 FileID/`dev+inode`、正式 500,000 publication 多样本与 Reference/Degradation Performance Gate、ranking/total/租约等 PRE_FREEZE 数值、AND/OR canonical 化和平台发行。
