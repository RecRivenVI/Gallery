...
当前补充状态（2026-07-31）：[EV-146](证据/验证记录.md) 已在 EV-136～EV-145 的列表/配置窗口、256 层深度和正式项数门禁上，修复规则正式 8 MiB 内容契约被通用 HTTP 1 MiB 解码上限提前截断的问题，并让管理端按 UTF-8 字节在解析、Schema/AJV 与保存前有界降级、保留超限原文；[EV-148](证据/验证记录.md) 又从真实管理 UI 把大于 1 MiB、低于 8 MiB 的草稿经真实 `galleryd` 保存并重载，Chromium/Firefox 独立专项各 1/1、完整链各 24/24。当前阶段 6 缺口已收窄为 8 MiB 内完整草稿解析/序列化/AJV/内存、无 Schema 项数上限的任意对象、真实设备与完整可访问性/弱网门禁；Web Gate 仍未通过。下文较早的 EV-136～EV-147 汇总应结合本段增量阅读。
...
[EV-127](证据/验证记录.md) 又把 Windows 外部工具执行放入带 `JOB_TIME`、`JOB_MEMORY` 与 `KILL_ON_JOB_CLOSE` 的 Job Object：父子树累计 CPU 时间耗尽会整体终止，父子聚合提交内存越界会被操作系统拒绝；默认 512 MiB、CPU 等于墙钟超时和 2 GiB/3,600 秒上限均冻结进 Job request，但仍属 PRE_FREEZE。非 Windows 当前没有等价的无竞态树级适配器，因此 external-tool capability 在持久 Job 前 fail-closed。本机真实 `ffprobe` 已在新限制下复验；恶意媒体语料库、非 Windows 门禁和整体 Security Gate 仍未完成。
