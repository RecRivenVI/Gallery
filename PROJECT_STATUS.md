...
| 阶段 4：查询与媒体 | 🟡（主线代码完成，部分参数未冻结） | 🟡（Correctness 与 500,000 publication 正式矩阵已完成；完整 Reference Gate 未过） | 搜索、排序、分页、显式规则/有效封面、媒体读取/下载、缩略图生成全部有代码闭环；EV-51/52 降低搜索与 publication 开销，EV-87/88 收窄聚合封面，EV-89/95 严格分页；EV-96～101 建立双关系、十目标来源、可续跑 publication 与 Query 正式入口，1%/10%/50% 各 20/20、总计 60/60 且失败 0，P95 为 2.168/2.147/5.581 ms | Query 并发/冷缓存、Degradation 仍未完成；超大合并图、排序权重、Total、租约、兼容版本策略和正式 API Freeze 尚未冻结 | 完成正式 500k Query/Degradation 矩阵并冻结接口 |
...
| 1 | 阶段 4 收尾 | publication 正式矩阵已完成；新建符合 500k/十目标来源/双关系形状的 Query 夹具，分窗执行冷/热并发与 Degradation 性能门禁并完成 API 接口冻结 | publication 1%/10%/50% 各 20/20、总计 60/60、失败 0，旧快照跨构建均可读且三个 P95 均低于 250 ms；完整 Reference Gate 仍取决于尚未执行的 Query/Degradation |
