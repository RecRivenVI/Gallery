...
| 阶段 4：查询与媒体 | 🟡（主线代码完成，部分参数未冻结） | 🟡（正确性已收口；500,000 publication 正式矩阵 42/60、失败 0） | 搜索、排序、分页、显式规则/有效封面、媒体读取/下载、缩略图生成全部有代码闭环；EV-51/52 降低搜索与 publication 开销，EV-87/88 收窄聚合封面，EV-89/95 严格分页；EV-96～101 建立双关系、十目标来源、可续跑 publication 与 Query 正式入口，当前 1%/10% 变化档各 20/20，50% 为 2/20 | publication 的 50% 变化档及 Query 并发/冷缓存、Degradation 仍未完成；超大合并图、排序权重、Total、租约、兼容版本策略和正式 API Freeze 尚未冻结 | 完成正式 500k 性能矩阵并冻结接口 |
...
| 1 | 阶段 4 收尾 | 继续完成正在运行的 500k publication 50% 变化档，再分窗执行 Query 冷/热并发与 Degradation 性能门禁并完成 API 接口冻结 | 当前 publication 为 42/60、失败 0；1%/10% 各 20/20，50% 为 2/20，正式多样本结果和接口数值仍未冻结 |
