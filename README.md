...
| 图形界面 | 响应式 Web/PWA 覆盖浏览、作品/媒体、实际封面与 CustomCover 编辑、Overlay、任务、规则、安全和维护页面 | 页面代码基线已实现；封面链路只有 mock 浏览器证据，完整浏览器门禁仍未通过 |
...
| 阶段 6：Web/PWA 界面 | 🟠（页面代码基线已实现；实时通道与写入口阻断已修复） | 🟡（Chrome/Edge 认证与实时通道主路径通过；完整写路径 E2E 仍未覆盖，正式 Gate 未通过） | 同源内嵌 Web/PWA 已覆盖浏览、同快照封面/媒体、CustomCover 编辑、Overlay、任务、规则、安全和维护页面骨架 | 封面链路尚无真实后端浏览器复验；完整管理写路径 E2E、Firefox、真实移动设备/屏幕阅读器与窄屏焦点陷阱未完成 | 扩大写路径与可访问性门禁，不进入桌面壳 |
