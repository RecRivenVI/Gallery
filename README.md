...
| 阶段 6：Web/PWA 界面 | 🟠（页面代码基线与首批真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实后端 E2E 已建立；正式 Gate 未通过） | 同源 Web/PWA 覆盖浏览与管理页面；主要业务/治理链已有双浏览器真实后端持续 E2E；EV-78 增加双入口 42rem 窄屏模态导航、焦点边界与当前页语义 | 真实存储取消与崩溃恢复响应、完整弱网抖动矩阵、真实移动/触控设备、人工屏幕阅读器与全页面可访问性未完成 | 扩大真实业务与可访问性门禁，不进入桌面壳 |
...
EV-78 已在当前 UI 上关闭窄屏导航焦点陷阱、Escape、焦点返还与响应式显隐缺口；证据仍是桌面 Chromium/Firefox 的移动 viewport 模拟，真实手机/平板、触控与人工屏幕阅读器继续列为未完成门禁。
