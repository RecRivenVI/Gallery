...
以下选择已用于正式代码和自动化测试，但仍可在不改变本规范行为的前提下兼容升级：

| 主题 | 当前实现 | 选择理由与边界 |
| --- | --- | --- |
| SQLite 驱动 | `modernc.org/sqlite` v1.53.0 | 纯 Go、基础发行不依赖 cgo；WAL、备份和两库测试必须持续通过 |
| Migration | 内嵌 forward-only runner | 只服务 SQLite；逐文件事务、版本表和 SHA-256 防历史改写，避免引入其他数据库驱动树 |
| OpenAPI 生成 | `oapi-codegen` v2.7.2 | 固定版本生成 Go model/client；只作开发工具，不进入生产运行依赖 |
| WebSocket | `github.com/coder/websocket` v1.8.15 | 基于 `net/http`、支持 `context.Context`、零传递依赖；事件语义仍由独立 Schema 拥有 |
| Web/PWA | React 19、TypeScript 5.9 strict、Vite 8、TanStack Query、React Router、OpenAPI 生成类型 | 构建产物嵌入 `galleryd`；运行时不依赖 Node；实现选择见 ADR-009 |
| 可访问 UI/规则表单 | React Aria Components、RJSF 6 + AJV | headless 交互原语与 Schema 表单；视觉语义由仓库 CSS token 拥有 |
| 结构化日志 | 标准库 `log/slog` JSON handler | 不引入日志框架；公共日志不得携带 secret、绝对媒体路径或 metadata 原文 |
| JSON Schema | `jsonschema/v6` v6.0.2 | 执行 Draft 2020-12 规则与协议 Schema；规范化和精确数字语义仍由 Gallery 层负责 |
| 测试基础 | 标准库 `testing`、`httptest` | 当前无需额外断言 DSL；真实 HTTP、WebSocket、SQLite 和临时 AppDirs 均可覆盖 |

Migration 曾对比通用工具，但当前工具会把多种远程数据库、容器和云驱动纳入模块图；Gallery v1 只有两个 SQLite 文件，因此选择窄化的仓库内 runner。该决定不冻结最终物理表结构，新增 migration 仍须兼容升级并在数据门禁中验证失败回滚。

## 明确不冻结的实现细节

本文不冻结 Go 包名、类/函数名、SQLite 表名、SQL Schema 升级框架、HTTP 路由的最终细粒度或视觉组件样式。阶段 6 当前前端技术组合按 ADR-009 接受，但仍可在不破坏 OpenAPI、同源资产版本、HTTP 事实源、可访问性和数据所有权的前提下替换。
