...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-91、EV-102、EV-113 | 🟡 | Vitest 15 个文件 211 项；Chromium/Firefox mock smoke 22/22；19 条路由在桌面/窄屏各 38 个状态通过 axe；隔离 Chromium 与 Firefox/真实 `galleryd` 完整运行器各 21 项 | 自动路由基线、作品身份交接、窄屏焦点及主要真实业务链已覆盖；真实移动/触控、人工屏幕阅读器、缩放/高对比、交互状态组合和其余弱网矩阵仍未完成 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-91 覆盖主要真实后端业务与治理持续链，EV-113 又把当前 19 条路由的桌面/窄屏稳定状态纳入双浏览器 axe。EV-92 的真实 Pixiv 证据属于独立 testlab，不是浏览器业务链；真实移动设备/触控、人工屏幕阅读器、缩放/高对比、交互状态组合与正式可用性 Gate 均未完成 |
...
| 3 | 阶段 6 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检，EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消，EV-113 已建立当前全路由桌面/窄屏自动 axe。下一步继续 HDD/SMB/NAS、publishing/崩溃恢复、其余弱网、交互状态组合、人工辅助技术与真实设备 | Pixiv 全量扫描保持低优先级，不能替代业务闭环和发布可用性门禁 |
