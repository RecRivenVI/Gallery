...
>
> [EV-148](Documents/证据/验证记录.md) 已补齐上述真实浏览器缺口：Chromium/Firefox 从管理 UI 创建独立规则包，完整提交大于 1 MiB、低于 8 MiB 的 JSON 草稿，经真实 `PUT`/`If-Match` 保存，并在页面重载后以 SHA-256 证明持久文本无损。独立专项各 1/1，完整真实后端链各 24/24，Source guard、恢复与 LAN 等价通过；8 MiB 内最坏结构、浏览器内存硬上限和真实设备仍未验证。
>
...
EV-148 把 EV-146 的前端与真实 HTTP 分层证据串成同一条真实 `galleryd` 浏览器链：管理 UI 创建服务端生成稳定 ID 的独立规则包，提交大于 1 MiB、低于 8 MiB 的合成 JSON，核对真实 `PUT`、`If-Match`、完整响应和页面重载后的 SHA-256。独立 Chromium/Firefox 各 1/1，加入正式运行器后的完整链各 24/24，Source guard、强杀/恢复、长停机与 LAN 均通过。首次完整链只暴露导航结束后 CDP response body 可能被回收的测试时序，改为对 UI 重载文本做摘要后稳定通过；没有以重试掩盖。该切片不证明 8 MiB 内最坏结构、浏览器内存硬上限或真实设备。
