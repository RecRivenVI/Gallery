...
| 阶段 6：Web/PWA 界面 | 🟠（页面代码基线与双入口设计重构已实现） | 🟡（隔离 Chromium/Firefox 真实后端 E2E 已建立；正式 Gate 未通过） | 同源 Web/PWA 覆盖浏览与管理页面；主要业务/治理链已有双浏览器真实后端持续 E2E；EV-78～EV-91 增加窄屏、弱网、双入口重构、媒体背压、Source 作者分页与授权 Job 历史连续加载 | 真实存储取消与崩溃恢复响应、其余完整弱网矩阵、真实移动/触控设备、人工屏幕阅读器与全页面可访问性未完成 | 扩大真实业务与可访问性门禁，不进入桌面壳 |
