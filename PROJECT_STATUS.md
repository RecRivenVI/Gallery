...
| 无障碍访问（键盘、屏幕阅读器等） | 需要 | React Aria + 语义 token、reduced motion、焦点与触控样式；forced-colors 服从系统调色板 | `web/src/design/*`、`web/src/gallery/components/chrome.tsx`、`web/src/manage/layout.tsx`、`web/e2e/gallery.spec.ts` | 🟠（EV-78/79 关闭窄屏导航焦点与 Grid 溢出；EV-113/116 建立全路由组合门禁；EV-117 增加五个关键交互状态） | Chromium/Firefox Playwright + axe：1280×800、390×844，以及 320×800 应用高对比 + forced-colors + WCAG 文本间距的路由和关键交互组合，`color-contrast` 保持启用 | EV-38～EV-40、EV-78、EV-79、EV-113、EV-116、EV-117 | 🟡 | 当前 19 条路由稳定状态和五个关键交互已覆盖；仍缺其余交互状态、人工屏幕阅读器、真实触控/虚拟键盘/安全区、实际 200%/400% 浏览器缩放和物理操作系统高对比门禁 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-91 覆盖主要真实后端业务与治理持续链，EV-113/116 又把当前 19 条路由的桌面/窄屏稳定状态及模拟强制颜色、文本间距、400% 等效重排纳入双浏览器 axe，EV-117 再补五个关键交互状态。EV-92 的真实 Pixiv 证据属于独立 testlab，不是浏览器业务链；真实移动设备/触控、人工屏幕阅读器、实际浏览器缩放、物理操作系统高对比、其余交互状态组合与正式可用性 Gate 均未完成 |
...
| 3 | 阶段 6 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检，EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消，EV-113/116 已建立当前全路由组合 axe，EV-117 已补五个关键交互状态。下一步继续 HDD/SMB/NAS、publishing/崩溃恢复、其余弱网/交互状态、人工辅助技术、真实缩放/系统高对比与真实设备 | Pixiv 全量扫描保持低优先级，不能替代业务闭环和发布可用性门禁 |
