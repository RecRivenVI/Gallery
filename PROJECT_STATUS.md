...
| 阶段 4：查询与媒体 | 🟡（主线代码完成，部分参数未冻结） | 🟡（Correctness、500,000 publication 及 Query warm/cold-process 矩阵已完成；完整 Reference Gate 未过） | 搜索、排序、分页、显式规则/有效封面、媒体读取/下载、缩略图生成全部有代码闭环；publication 1%/10%/50% 各 20/20、总计 60/60 且 P95 为 2.168/2.147/5.581 ms；EV-150/151 收敛宽标题前缀与并发重复 Total，同形状 Query warm/cold-process 各 63/63、失败/超时 0 | 查询分类/候选量正式阈值、结构化过滤计划、Degradation、维护/哈希预算仍未完成；超大合并图、排序权重、Total、租约、兼容版本策略和正式 API Freeze 尚未冻结 | 冻结 Query 阈值与计划门禁，执行 Degradation/维护/哈希矩阵并冻结接口 |
...
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口、设计重构与主要真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、媒体优先画廊端与紧凑管理端；EV-54～EV-91 覆盖主要业务/治理/恢复与弱网切片，EV-102 部分采纳共享动效基座，EV-113～EV-118 建立全路由/关键状态可访问性组合，EV-129/132 清除全部依赖审计例外，EV-136～EV-148 已窗口化主要大列表与规则配置、对齐 256 层深度、验证 4,096/10,000 正式项数，并完成大于 1 MiB 规则正文的真实浏览器保存/重载 | 浏览器业务门禁仍使用合成 Source；EV-92 只补到独立 testlab 的真实 Pixiv 有界预检。真实存储浏览器链、其余弱网矩阵、真实缩放/系统高对比、触摸设备与屏幕阅读器未验证；8 MiB 内完整草稿解析/序列化/AJV/内存仍未验证 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
...
EV-147 完成 publication 的正式 Reference 形状：500,000 Work、十权威目标来源、每 Work 两条 Creator 关系，1%/10%/50% 各 20 个样本全部成功，总计 60/60、0 failure/timeout，旧 active publication 在全部构建期间可读；Publish P95 为 2.168/2.147/5.581 ms，均低于 250 ms。36 小时受限运行的最终报告已封印 SHA-256；该 publication 子矩阵通过。报告仍为 warm cache 合成完整 Catalog，500k Query warm/cold-process 并发与 Degradation 未执行，所以完整 Reference Gate/API Freeze 尚未通过。
...
EV-142 为 Session、API Token、Share、本地账户与 Grant 增加默认 50、上限 200 的 live keyset 页面，cursor 绑定资源类型，并在存在主体过滤时绑定 Principal/目标主体，由 control v26 五条索引支撑；OpenAPI/生成客户端同步 `limit`/`cursor`/`nextCursor`。管理端五张表只挂载当前页，写入或实时变化后重置旧分页族。五类各 55 条应用分页、HTTP 越界/跨作用域回归、管理组件 48/48、Chromium/Firefox 51 条 mock 定向 2/2、完整 mock 34/34 及 1283.1 秒根级 219 项 Vitest/全仓门禁通过；同规模真实后端浏览器专项、客户端缓存上限、配置数组、真实设备与完整 Security/Web Gate 仍未完成。

EV-143 为当前规则编辑器的 RJSF 数组、参数 Schema 属性、tests、extensions、递归 JSON 对象/数组和字段撤销列表建立每页最多 20 项的本地挂载窗口，数据仍保留在同一份无损草稿，全局索引/顺序与服务端权威不变。同轮修复 RJSF `:read-only` 误命中可编辑 `select` 导致的 4:1 对比度缺口。管理组件 52/52、Chromium/Firefox 定向 2/2、完整 mock 36/36 与 1055.4 秒根级 223 项 Vitest/全仓门禁通过。边界夹具仅为 21 项，不限制完整草稿内存，也不代表 4,096/10,000 极限、恶意超深 JSON、同规模真实后端、真实设备或完整 Web Gate 已通过。

EV-144 让前端结构化 JSON 在递归挂载前以显式栈核对后端权威 256 层容器边界，越界子树只显示警告并保留无损原始 JSON；Go 跨语言测试锁定前后端常量。同轮完整 smoke 在 Chromium 捕获 Modal 进入态祖先 opacity 导致的 3.54:1 对比度，改为遮罩背景色和内容位移过渡后收口。管理组件 54/54、15 文件 225 项 Vitest、双浏览器深度/高对比定向各 2/2、最终 mock 36/36 与 1068.1 秒根级全仓门禁通过。浏览器只覆盖完整规则 257 层越界，不限制完整草稿解析/内存，也不代表 4,096/10,000 项极限、真实后端同规模、真实设备或完整 Web Gate 已通过。

EV-145 把规则 Schema 的正式数组上限纳入持续门禁：同一草稿包含 4,096 个 primitive 与 10,000 个 test 时，两类编辑器各只挂载当前 20 项，切回无损 JSON 文本后末项完整存在。定向组件 1/1、Chromium/Firefox 2/2、15 文件 226 项 Vitest、完整 mock 36/36 与 1087.5 秒根级全仓门禁通过。该证据不限制完整草稿解析、序列化、AJV 或浏览器内存，也不覆盖无 Schema 项数上限的对象、同规模真实后端、真实设备或完整 Web Gate。

EV-146 修复规则正式 8 MiB 内容上限被通用 HTTP 1 MiB 解码器提前截断的问题。普通 JSON API 继续保持 1 MiB；规则端点独立覆盖双精确文档最坏 JSON 转义，单份内容仍由应用层按 8 MiB 拒绝。管理端按 UTF-8 字节在 Lossless JSON、Schema/AJV 与保存前有界降级并保留原文，前后端常量由 Go 测试锁定。真实 HTTP 已验证大于 1 MiB 的 import/save 和大于 8 MiB 的拒绝；组件 2/2、双浏览器生产资产 4/4、16 文件 228 项 Vitest、mock 38/38、Windows/WSL2 race 与 1385.1 秒根级门禁通过。仍未从真实 `galleryd` 浏览器提交同一超大正文，也未证明前端内存或 8 MiB 内最坏形状。

EV-148 从真实管理 UI 创建独立 RulePackage，把大于 1 MiB、低于 8 MiB 的完整 JSON 草稿经真实 `galleryd`、`PUT` 和 `If-Match` 保存，并在页面重载后以 SHA-256 核对服务端持久正文。Chromium/Firefox 独立专项各 1/1；加入正式运行器后的完整链各 24/24，Source guard、强杀/恢复、长停机与 LAN 均通过。首次完整 Chromium 运行只发现导航后 CDP response body 回收导致的测试时序，改为 UI 重载文本摘要后两浏览器全链稳定通过；该证据不限制 8 MiB 内最坏结构、浏览器内存或真实设备。

EV-149 关闭“输入未超 8 MiB、但 Import/Compile 规范化结果越界”的自相矛盾状态：输入与 canonical 输出都执行同一正式上限，8 MiB−1 草稿可保存为无效修订且保留原文，缩减后可继续验证、重载。实现提交 `7d34a30` 的 Windows/WSL2 定向、双浏览器专项和 532.2 秒根级检查通过；完整 24/24 浏览器链未重复执行。

EV-150 为全授权、无过滤、title asc/desc、非数字结尾的可证明前缀形态增加 NaturalSortKey v2 有界范围页读，并以有界 publication 精确值索引保留 Filename/Creator/Tag 更高档位排序。实现提交 `79246fa` 后，500,000 正式 warm 矩阵的九个 `wide-cjk` 组合全部通过；当轮仍在 42/63 被 filename/structured 高并发超时阻断，因此只关闭宽标题路径。

EV-151 为不可变 publication Total 增加 256 条硬上限、错误不保留、并发首次 miss 合并的语义缓存；授权 Source 集合、规范查询/过滤与 Total 协议进入指纹，依赖当前 control.db 等价组的 `creator.id` 绕过缓存。实现提交 `1a21bec` 后，同一 500,000 Work/十来源/双关系 Catalog 的 warm 与 cold-process 正式全矩阵均为 63/63、0 failure/timeout。最慢 filename limit 200/concurrency 16 的 warm P95/P99 为 22.807/26.264 秒，cold-process 为 23.501/27.486 秒；当前 30 秒请求边界内完成，但分类/候选量 P95 阈值、结构化过滤 EXPLAIN、冷存储与 Degradation 尚未冻结，完整 Reference/API Freeze 仍未通过。

EV-152 在正式复跑前冻结 `query-reference-500k-p95-v1`，让每个组合显式绑定类别、limit、候选量、原文复核、concurrency 与 P95 预算，并为 structured-and/or/overlay-favorite 增加生产 SQL builder 的计划门禁。首轮正式 warm 没有被“整体完成”掩盖：filename limit 200/concurrency 16 的 1/100 超时使样本完整性、P99 与 P95 finding 共 3 项失败。

EV-153 只合并精确 SQL/实参相同且同时在途的 immutable page 计算，最多 256 个键，完成后不保留页面缓存；结果深拷贝，最后 waiter 取消才终止 SQL。正式 warm 因此 63/63、130 findings 全绿；cold-process 又准确暴露 structured-and limit 20/concurrency 4、16 的 P95 分别为 2269.533/2652.247 ms，超过固定 1750/2500 ms 预算。

EV-154 把 Total 候选预算从 10,000 收紧至 5,000，并将实际预算与协议版本 1 纳入矩阵身份；这仍是 PRE_FREEZE 候选，不改变 total 三态。最终正式 warm/cold-process 各 63/63、130 findings、0 failure/timeout，报告 SHA-256 分别为 `B75531E3F10E56E3B10BCEE14AC4F0768E8D70DC08080E79D60D8C386DE9F894` 与 `6856900EAA22CDC1E4DFD8827D952EB2C0F3B71400B7795DDCBECFEB53FCD64F`。Query Reference 子矩阵通过；cold-process 不清 OS 文件缓存，相同页并发不代表混合查询吞吐，维护、完整哈希与 Degradation 仍使完整 Reference/API Freeze 保持未通过。
...
| Pixiv 真实 Source 只读有界预检 | RC 最终全量验收前置 | 只做规则转换、注册、45 秒 `index` 取消、恢复重跑与全树 guard，为候选 RC 的最后一道全量验收准备安全执行基础 | `tools/testlab/cmd/{rulesimport,probe}`、`tools/testlab/stages/sourcelab`、`internal/scanner` | ✅（有界预检范围） | 真实 SSD Source、独立 AppDirs、动态 loopback、低资源运行 | EV-92～EV-94 | ✅（本地 SSD/index discovery 有界范围） | 370,712 文件/105,202 目录前后零变化；取消 POST 后 201 ms 观察终态；未完成候选 RC 上的全量扫描/哈希/发布、完整前端可用性及其它存储类型 |
...
|---|---|---|---|---|---|---|
| 全部 Go 测试（816 个顶层 `Test*`/`Benchmark*`/`Example*` 函数，覆盖 67 个目录/包） | 贯穿各阶段 | `cmd`/`internal`/`pkg`/`tools` 共 191 个 `*_test.go` 文件 | `scripts/Check.ps1` 每次运行 | ✅（在 CI 上持续运行并要求全部通过） | Windows + Ubuntu（GitHub Actions） | 绝大多数是模拟/合成数据；部分显式门禁使用真实 OS/工具/Source，局限分别登记；Argon2id benchmark 与恶意媒体真实工具门禁需显式启用 |
| 数据库迁移（control 26 个迁移文件，catalog 20 个迁移文件） | 阶段 0-6 | `internal/storage/migrations/{control,catalog}` | EV-12 及各阶段收尾记录、EV-37、EV-42、EV-44、EV-46、EV-51、EV-52、EV-59、EV-87、EV-89、EV-91、EV-141、EV-142 | ✅ | 空库/旧库升级单元测试；control v23 增加 Creator NaturalSortKey，v24 增加 Job 历史 keyset 索引，v25 增加结构决策历史三条 keyset 索引，v26 增加五类安全资源 keyset 索引；catalog v20 增加 Creator/Source 封面窄候选 | 最终物理 Schema 仍未冻结；旧规则封面只能近似回填，需重扫精确恢复；规则隐藏/角标需重扫；正式变化 publication 仍待完整 Reference Gate |
...
| 性能测试（Reference Performance） | 阶段 3、4 | `internal/query/reference_performance_test.go` 微基准、`tools/testlab` 生产 Store/HTTP 矩阵及 `publication-perf`（均需显式运行） | EV-23/35/36/51/52/96～101/147/150～154 均明确区分方向性测量、执行器预检、失败诊断与正式 Gate | 🟡 | 500k/十来源/双关系 publication 1%/10%/50% 各 20/20；同形状 Query profile v1 warm/cold-process 各 63/63、130 findings 全绿，报告均有原子身份与 SHA-256 | publication 与 Query Reference 子矩阵已通过；`TotalBudget=5000` 仍 PRE_FREEZE，混合查询吞吐、冷存储、维护、完整哈希和 Degradation 尚未完成，不能当作完整门禁结论 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-91、EV-102、EV-113、EV-116～EV-118、EV-129、EV-132、EV-136～EV-148 | 🟡 | Vitest 16 个文件 228 项；Chromium/Firefox mock smoke 38/38；19 条路由在桌面/窄屏各 38 个状态通过 axe，并在 320px 模拟强制颜色/文本间距下覆盖路由及五个 mock 关键交互；精确生产资产上的 Chromium/Firefox 完整真实运行器各 24/24，full/production 依赖审计为 0 漏洞/0 例外；EV-136～EV-148 已限制主要大列表与规则配置挂载、对齐 256 层/正式项数/8 MiB 边界并补齐 HTTP 会话失效收敛 | 自动路由基线、作品身份交接、窄屏焦点、主要大列表、规则配置窗口/深度守卫/正式项数/8 MiB 边界及主要真实业务链已覆盖；真实移动/触控、人工屏幕阅读器、实际缩放/物理高对比、8 MiB 内完整草稿解析/序列化/AJV/内存、交互状态及弱网矩阵仍未完成 |
...
| 1 | 阶段 4 收尾 | publication 与 Query Reference 子矩阵已通过；执行混合查询吞吐/冷存储 Degradation、维护/哈希预算并完成 API 接口冻结 | publication 60/60；Query profile v1 warm/cold-process 各 63/63、130 findings 全绿；完整 Reference Gate 仍取决于 Degradation/维护/哈希与 PRE_FREEZE 数值收口 |
| 2 | 阶段 5 | 完成真实 LAN 多设备与目标低端设备 Argon2id 延迟/并发验证 | 同机 Chrome/Edge 和高性能工作站证据已取得，剩余缺口需要外部设备环境 |
| 3 | 阶段 6 与 RC 最终验收 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检，EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消，EV-113/116 已建立当前全路由组合 axe，EV-117 已补五个关键交互状态，EV-118 又把其中的双入口配对与管理安全/维护状态接入真实后端，EV-129/132 已清除全部依赖审计例外，EV-136～EV-149 已限定主要大列表与规则配置挂载规模、对齐 256 层容器边界与 4,096/10,000 正式项数，并闭合 8 MiB 规则正文的真实浏览器与 canonical 边界。下一步继续 8 MiB 内完整草稿资源响应、HDD/SMB/NAS、publishing/崩溃恢复、弱网/交互状态、人工辅助技术、真实缩放/系统高对比与真实设备 | 其它 RC 开发与门禁完成后，以候选 RC 执行 Pixiv 全量只读扫描、完整规则/媒体可用性和 Source 零写入验收；这是最后一道必过门槛，未通过不得交付 RC |
