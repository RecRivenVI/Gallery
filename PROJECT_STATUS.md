...
| 阶段 3：扫描、任务与目录库 | ✅（代码与模拟数据层面） | 🟡（真实大盘抽样与 Pixiv 有界预检通过，全量未完成） | SSD/HDD 各完成几十万文件规模抽样；EV-92～EV-94 又在真实 Pixiv 370,712 文件/105,202 目录上完成规则接入、45 秒有界 `index`、201 ms 内观察取消终态、同 AppDirs 恢复重跑与全树零写入 guard | Pixiv/真实盘全量扫描与哈希、活动 Hash/HDD/SMB/NAS 取消、正式性能门禁和网络共享盘尚未完成 | 完成全量运行准备，同时优先收口阶段 4 正式压力门禁 |
...
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携测试制品、同源切换及损坏备份回滚主路径已建立） | 🟡（本地独立制品、切换/恢复及损坏备份 smoke 通过，正式 Gate 未通过） | EV-103 从精确干净提交构建同源双前端的 Windows x64 ZIP；EV-104 用两个独立版本标签完成事实承接和正常恢复；EV-105 又证明损坏备份时保留当前库、消费标记并记录失败，程序目录持续零变化 | Authenticode、安装/更新、CredentialStore、真实历史升级/降级、磁盘满/权限/中断回滚、平台矩阵及桌面壳均未完成 | 先完成签名、历史升级/降级、其余失败回滚和 Windows RC 门禁 |
...
EV-105 再把损坏备份失败回滚接入相同的真实便携制品探针：正常恢复后创建第二份备份与备份后 Library，登记恢复、优雅停止并只在临时 AppDirs 内损坏备份；新进程必须继续读到当前库中的备份后事实、消费 pending 标记并记录 `applied=false`。精确干净提交 `61211f2` 的 `0.1.9-ev105`/`0.2.0-ev105` 两包均为 `dirty=false`、`unsigned`，程序树封印和全部优雅停止继续通过。历史 Schema 升级/降级、磁盘满、权限失败和原子落位中断仍未验证，正式 RC Gate 不变。
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| Windows 正式支持（真实运行、升级、崩溃恢复、签名） | 需要 | 增加无壳便携测试制品、双版本切换/恢复及损坏备份回滚主路径作为 RC 前基线 | `pkg/galleryversion`、`scripts/{Build,Test}-WindowsPortable.ps1`、`scripts/Test-WindowsPortableUpgrade.ps1`、`tools/testlab/cmd/portable-upgrade`、Windows CI | 🟠 | 精确干净提交构建、版本/摘要/SBOM/签名事实、动态 loopback、同 AppDirs 强杀重启、双标签事实承接、control 恢复和损坏备份保留当前库 | EV-103～EV-105 | 🟡（本地制品、同源切换及损坏备份 smoke 通过） | 仍未完成正式签名、安装/更新、真实历史升级/降级、磁盘满/权限/中断回滚、低权限/多用户与完整 Windows 平台门禁 |
...
| 安装包、签名、SBOM（软件物料清单）、依赖安全检查 | 需要 | 先建立 Windows x64 便携 ZIP、同源切换/恢复与损坏备份回滚门禁，再完成安装与签名 | EV-103 已生成三个 CycloneDX SBOM、清单、包内/外 SHA-256，构建器支持 Authenticode fail-closed；EV-104/105 让两个独立包先各自验包再切换、正常恢复并拒绝损坏备份；CI 继续执行依赖审计/漏洞扫描 | 🟠（部分） | 本地正向/负向制品 smoke、双版本切换/恢复与损坏备份回滚；Windows CI 与手工来源证明 workflow | EV-103～EV-105、`ci.yml`、`windows-portable.yml` | 🟡 | 当前精确包仍为 `unsigned`；无安装器、证书/时间戳、自动更新、真实历史迁移或签名后平台证据，不能作为 RC |
...
| Windows/Linux/macOS/Docker 平台支持 | 阶段 7 | Windows 增加本地便携测试包独立启动/强杀重启、同源双标签切换/恢复及损坏备份回滚；CI 仍只有 Windows + Ubuntu | EV-103～EV-105、ADR-007 四级支持成熟度表 | 🟡（Windows 增加制品/切换/损坏备份 smoke，Linux 仍停在 CI 层级）/ ⏳（macOS/Docker） | 当前工作站 Windows x64 + GitHub Actions 配置 | 未达到"发行候选"或"正式支持"级别；本地无签名包与 CI Linux 都不能外推为正式平台支持 |
...
| 发布/签名/SBOM | 阶段 7 | Windows 便携包构建/验证、三份 CycloneDX SBOM、清单、包内/外摘要、可选 Authenticode、双版本切换/恢复及损坏备份回滚已进入脚本与 CI | EV-103～EV-105、`scripts/{Build,Test}-WindowsPortable.ps1`、`scripts/Test-WindowsPortableUpgrade.ps1`、`windows-portable.yml` | 🟡（未签名测试基线） | 本地精确干净提交 ZIP 正向/负向、同源双标签切换及损坏备份 smoke；远端 workflow 待当前提交触发 | 正式证书/时间戳、安装器、更新、真实历史升级/降级、磁盘满/权限/中断回滚与 RC 仍不存在 |
...
| 发行缺口 | EV-103 已有未签名 Windows 便携 ZIP、三个 SBOM、摘要与 smoke；EV-104/105 再覆盖同源双标签事实承接、正常恢复和损坏备份保留当前库。仍没有安装包、正式代码签名/时间戳、自动更新、CredentialStore、真实历史升级/降级、磁盘满/权限/中断回滚和平台支持证据 |
...
| 4 | 阶段 7 | 在 EV-103～105 便携、同源切换与损坏备份回滚基线上完成 Authenticode、真实历史升级/降级、磁盘满/权限/中断回滚与 Windows RC；随后推进 Linux 原生、macOS、Docker、SMB/NAS 和安装发行 | 当前只关闭制品编排、程序/数据分离、正常恢复和损坏备份拒绝；签名、历史迁移、其余失败回滚和平台支持仍是正式发行阻断项 |
