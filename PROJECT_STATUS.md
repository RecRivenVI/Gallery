...
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携测试制品与同源切换主路径已建立） | 🟡（本地独立制品、切换与恢复 smoke 通过，正式 Gate 未通过） | EV-103 从精确干净提交构建同源双前端的 Windows x64 ZIP；EV-104 用两个独立版本标签完成用户事实承接、control 备份验证与同 AppDirs 重启恢复，并封印程序目录零变化 | Authenticode、安装/更新、CredentialStore、真实历史升级/降级与失败回滚、平台矩阵及桌面壳均未完成 | 先完成签名、历史升级/失败回滚和 Windows RC 门禁 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| Windows 正式支持（真实运行、升级、崩溃恢复、签名） | 需要 | 增加无壳便携测试制品与双版本切换/恢复主路径作为 RC 前基线 | `pkg/galleryversion`、`scripts/{Build,Test}-WindowsPortable.ps1`、`scripts/Test-WindowsPortableUpgrade.ps1`、`tools/testlab/cmd/portable-upgrade`、Windows CI | 🟠 | 精确干净提交构建、版本/摘要/SBOM/签名事实、动态 loopback、同 AppDirs 强杀重启、双标签事实承接与 control 恢复 | EV-103、EV-104 | 🟡（本地制品及同源切换 smoke 通过） | 仍未完成正式签名、安装/更新、真实历史升级/降级、失败回滚、低权限/多用户与完整 Windows 平台门禁 |
...
| 安装包、签名、SBOM（软件物料清单）、依赖安全检查 | 需要 | 先建立 Windows x64 便携 ZIP 与同源切换/恢复门禁，再完成安装与签名 | EV-103 已生成三个 CycloneDX SBOM、清单、包内/外 SHA-256，构建器支持 Authenticode fail-closed；EV-104 让两个独立包先各自验包再切换/恢复；CI 继续执行依赖审计/漏洞扫描 | 🟠（部分） | 本地正向/负向制品 smoke、双版本切换/恢复；Windows CI 与手工来源证明 workflow | EV-103、EV-104、`ci.yml`、`windows-portable.yml` | 🟡 | 当前精确包仍为 `unsigned`；无安装器、证书/时间戳、自动更新、真实历史迁移或签名后平台证据，不能作为 RC |
...
| Windows/Linux/macOS/Docker 平台支持 | 阶段 7 | Windows 增加本地便携测试包独立启动/强杀重启及同源双标签切换/恢复；CI 仍只有 Windows + Ubuntu | EV-103、EV-104、ADR-007 四级支持成熟度表 | 🟡（Windows 增加制品与切换 smoke，Linux 仍停在 CI 层级）/ ⏳（macOS/Docker） | 当前工作站 Windows x64 + GitHub Actions 配置 | 未达到"发行候选"或"正式支持"级别；本地无签名包与 CI Linux 都不能外推为正式平台支持 |
...
| 发布/签名/SBOM | 阶段 7 | Windows 便携包构建/验证、三份 CycloneDX SBOM、清单、包内/外摘要、可选 Authenticode 及双版本切换/恢复已进入脚本与 CI | EV-103、EV-104、`scripts/{Build,Test}-WindowsPortable.ps1`、`scripts/Test-WindowsPortableUpgrade.ps1`、`windows-portable.yml` | 🟡（未签名测试基线） | 本地精确干净提交 ZIP 正向/负向及同源双标签切换 smoke；远端 workflow 待当前提交触发 | 正式证书/时间戳、安装器、更新、真实历史升级/降级、失败回滚与 RC 仍不存在 |
...
| 发行缺口 | EV-103 已有未签名 Windows 便携 ZIP、三个 SBOM、摘要与 smoke；EV-104 再覆盖同源双标签事实承接、备份验证和重启恢复。仍没有安装包、正式代码签名/时间戳、自动更新、CredentialStore、真实历史升级/降级、损坏备份/磁盘满失败回滚和平台支持证据 |
...
| 4 | 阶段 7 | 在 EV-103/104 便携与同源切换基线上完成 Authenticode、真实历史升级/降级、失败回滚与 Windows RC；随后推进 Linux 原生、macOS、Docker、SMB/NAS 和安装发行 | 同源双标签切换只关闭制品编排、程序/数据分离与恢复主路径，签名、历史迁移、失败回滚和平台支持仍是正式发行阻断项 |
