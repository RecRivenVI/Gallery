...
func pairedRuleServer(t *testing.T) (*httptest.Server, *http.Client, string) {
	...
	server := httptest.NewServer(httpapi.New(config.ModePersonal, store, fixedClock, personal, resources, nil, nil, nil, nil, slog.New(slog.NewJSONHandler(io.Discard, nil))))
