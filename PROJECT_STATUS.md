...
| 阶段 4：查询与媒体 | 🟡（主线代码完成，部分参数未冻结） | 🟡（正确性已收口；500,000 publication 正式矩阵 40/60、失败 0） | 搜索、排序、分页、显式规则/有效封面、媒体读取/下载、缩略图生成全部有代码闭环；EV-51/52 降低搜索与 publication 开销，EV-87/88 收窄聚合封面，EV-89/95 严格分页；EV-96～101 建立双关系、十目标来源、可续跑 publication 与 Query 正式入口，当前 1%/10% 变化档各 20/20 | publication 的 50% 变化档及 Query 并发/冷缓存、Degradation 仍未完成；超大合并图、排序权重、Total、租约、兼容版本策略和正式 API Freeze 尚未冻结 | 完成正式 500k 性能矩阵并冻结接口 |
...
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口、设计重构与主要真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、媒体优先画廊端与紧凑管理端；EV-54～EV-91 覆盖主要业务/治理/恢复与弱网切片，EV-102 部分采纳共享动效基座，EV-113～EV-118 建立全路由/关键状态可访问性组合，EV-129 清除生产路由审计例外并在最终资产上重跑双浏览器完整 23 项链 | 浏览器业务门禁仍使用合成 Source；EV-92 只补到独立 testlab 的真实 Pixiv 有界预检。真实存储浏览器链、其余弱网矩阵、真实缩放/系统高对比、触摸设备与屏幕阅读器未验证；画廊端无 DOM 虚拟化 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携、恢复/回滚及首条真实历史升级边界已建立） | 🟡（当前双前端制品、schema 23→24/反向拒绝、包内、进程级及当前用户 ACL 恢复失败门禁通过，正式 Gate 未通过） | EV-103～105 完成便携、同源切换、正常/损坏备份；EV-106 证明真实 schema 23→24、旧程序拒绝新 migration 与当前程序复启；EV-107/110 覆盖轮换/落位拒绝；EV-109/119 关闭落位后连续性缺口；EV-121～123 关闭当前库缺失、安全收尾和状态文件失败；EV-124/125 关闭便携同进程双 Rename 与 finalize 强杀续接；EV-130 生成当前双前端未签名测试包，EV-131 再以真实 NTFS ACL 拒绝轮换并生成精确干净 `0.3.1-ev131` 包 | Authenticode、安装/更新、CredentialStore、更多支持版本跨度、磁盘满、低完整性/多账户/继承 ACL、其它恢复窗口强杀/真实断电、平台矩阵及桌面壳均未完成 | 先完成签名、其余失败回滚和 Windows RC 门禁 |
...
| Windows/Linux/macOS/Docker 平台支持 | 阶段 7 | Windows 增加本地便携测试包、恢复/回滚、真实 schema 23→24/反向拒绝、轮换/候选落位失败、包内及便携进程双失败、当前库缺失/stale incoming、安全收尾、状态文件失败、finalize 强杀及当前用户 NTFS ACL 门禁；CI 仍只有 Windows + Ubuntu | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-125、EV-130、EV-131、ADR-007 四级支持成熟度表 | 🟡（Windows 增加制品/恢复/升级/多条真实失败门禁，Linux 仍停在 CI 层级）/ ⏳（macOS/Docker） | 当前工作站 Windows x64 + GitHub Actions 配置 | 未达到"发行候选"或"正式支持"级别；本地无签名包，Windows 本机结果与 CI Linux 都不能外推为正式平台支持 |
...
| 发布/签名/SBOM | 阶段 7 | Windows 便携包、SBOM、清单、摘要、可选 Authenticode、恢复/回滚及真实历史升级边界已进入脚本与 CI；EV-109/110/119/121～125 补齐多条连续性、双失败、状态文件和强杀门禁，EV-131 再补当前用户 NTFS ACL 拒绝 | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-125、EV-130、EV-131、Windows 发行脚本与 workflow | 🟡（未签名测试基线） | 本地精确干净 ZIP 正向/负向、正常/损坏备份、schema 23→24/反向拒绝、轮换/候选落位失败、包内/便携进程双失败、当前库缺失/stale incoming、安全收尾、状态文件失败、精确强杀窗口及 ACL 拒绝的进程 fail-closed/重试；远端 workflow 待当前提交触发 | 正式证书/时间戳、安装器、更新、更多支持版本跨度、磁盘满、低完整性/多账户/继承 ACL、其它恢复窗口强杀/真实断电及 RC 仍不存在 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-91 覆盖主要真实后端业务与治理持续链，EV-113/116 又把当前 19 条路由的桌面/窄屏稳定状态及模拟强制颜色、文本间距、400% 等效重排纳入双浏览器 axe，EV-117 再补五个关键交互状态，EV-118 把双入口配对及管理安全/维护关键状态接入真实后端并修复换壳瞬态语义，EV-129 清除 production 路由审计例外并在精确生产资产上重跑双浏览器完整 23 项链。EV-92 的真实 Pixiv 证据属于独立 testlab，不是浏览器业务链；真实移动设备/触控、人工屏幕阅读器、实际浏览器缩放、物理操作系统高对比、其余交互状态组合与正式可用性 Gate 均未完成 |
...
| 发行缺口 | EV-103 已有未签名 Windows 便携 ZIP、三个 SBOM、摘要与 smoke；EV-104～125 覆盖正常/损坏备份、真实 schema 23→24/反向拒绝、轮换/落位/连续性/状态文件失败及一个强杀窗口，EV-130 刷新当前双前端制品，EV-131 再关闭当前用户本地 NTFS ACL 拒绝切片。仍没有安装包、正式代码签名/时间戳、自动更新、CredentialStore、完整支持版本跨度、磁盘满、低完整性/多账户/继承 ACL、其它恢复窗口强杀/真实断电和平台支持证据 |
...
| 1 | 阶段 4 收尾 | 继续完成正在运行的 500k publication 50% 变化档，再分窗执行 Query 冷/热并发与 Degradation 性能门禁并完成 API 接口冻结 | 当前 publication 为 40/60、失败 0；正式多样本结果和接口数值仍未冻结 |
| 2 | 阶段 5 | 完成真实 LAN 多设备与目标低端设备 Argon2id 延迟/并发验证 | 同机 Chrome/Edge 和高性能工作站证据已取得，剩余缺口需要外部设备环境 |
| 3 | 阶段 6 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检，EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消，EV-113/116 已建立当前全路由组合 axe，EV-117 已补五个关键交互状态，EV-118 又把其中的双入口配对与管理安全/维护状态接入真实后端，EV-129 清除 production 路由审计例外并重跑双浏览器完整真实链。下一步继续 HDD/SMB/NAS、publishing/崩溃恢复、其余弱网/交互状态、人工辅助技术、真实缩放/系统高对比与真实设备 | Pixiv 全量扫描保持低优先级，不能替代业务闭环和发布可用性门禁 |
| 4 | 阶段 7 | 在 EV-103～131 的便携、恢复/回滚、真实 schema 23→24 与当前用户 NTFS ACL 门禁上完成 Authenticode、支持版本跨度、磁盘满、低完整性/多账户/继承 ACL、其它恢复窗口强杀/真实断电及 Windows RC；随后推进 Linux 原生、macOS、Docker、SMB/NAS 和安装发行 | 当前已关闭制品编排、多条恢复失败、一个精确强杀窗口及当前用户本地 ACL 拒绝；签名、其余权限/异常和平台支持仍是正式发行阻断项 |
