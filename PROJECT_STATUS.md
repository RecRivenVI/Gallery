...
| 阶段 5：账户、安全与多客户端 | 🟠（代码与合成安全收尾已实现；恶意输入缺陷已收口） | 🟡（Personal、同机 LAN 与真实外部测试进程树补证，正式 Gate 未通过） | EV-37/EV-38/EV-44/EV-48 的安全收口之外，EV-60 已把 Session、API Token、Share、allow/deny Grant、账户停用恢复和精确 Session 吊销接入隔离真实浏览器；EV-86 又让 Creator/Library 聚合封面遵守 deny、Token scope 与独立 `media.read`；EV-114 证明输出溢出/超时会终止真实 OS 进程树 | 真实 LAN 多设备、目标低端设备 Argon2id、真实恶意媒体容器/生产外部工具及 OS 级 CPU/内存门禁仍未完成；同机 loopback 不能替代正式 Security Gate | 完成外部设备与恶意资源门禁 |
...
| 威胁模型测试（路径攻击、恶意元数据、恶意媒体文件、限流） | 需要 | 认证爆破、Host/Origin/CSRF/Content-Type、WS 防滥用，以及路径/metadata/媒体正文/恢复输入均进入正式合成测试；真实测试二进制覆盖外部工具输出溢出、超时和进程树强杀 | `internal/auth/security_test.go`、`internal/transport/httpapi/security_api_test.go`、`internal/contract/realtime/envelope_test.go`、`internal/toolrunner/integration_test.go`、`internal/platform/process/controller_test.go`、`tools/testlab/stages/stage5/security` | ✅（合成攻击矩阵 + 真实 OS 进程树切片） | 合成安全回归 + 高重复/race + Windows/WSL2 真实进程树 | EV-09、EV-37、EV-114 | 🟡 | 没有真实恶意媒体容器、生产 ffmpeg/ffprobe Resolver 或 OS 级 CPU/内存硬限额，也不代表真实 LAN；整体 Security Gate 未通过 |
...
| 安全（认证、授权、Web 边界、路径穿越、恶意元数据/媒体、限流） | 阶段 5 | 正式生产包覆盖账户/Token/Grant/Session/WS/Web 与合成攻击；Work 查询逐成员授权；真实 Chrome/Edge 已覆盖 Personal/LAN 主路径和吊销；外部工具输出/超时已覆盖真实 OS 进程树终止 | EV-09、EV-37、EV-38、EV-44、EV-60、EV-114 | 🟡 | Windows 合成与浏览器；Windows 真实进程树重复；WSL race；隔离 Personal/LAN Chromium 安全管理链 | 真实物理 LAN 多设备、目标低端设备、真实恶意媒体容器/生产外部工具与 CPU/内存硬限额未完成，整体 Gate 未通过 |
...
| 已修正的外部工具测试漂移（EV-114） | 已新增 Argon2id benchmark，但只在当前工作站取得 EV-38 数据且 CI 从不执行；全仓库仍无正式 Fuzz。此处旧文称 `internal/toolrunner` 的「有界输出/超时/强杀」测试从未真正触及上限，但提交 `2af146a` 早已让 1,024-byte 输出上限和 3 秒超时驱动真实父子孙进程树终止，EV-114 又在 Windows 重复与 WSL2 race 复验 | 已关闭“上限未被触及”这一文档错误；生产 Resolver、真实 ffmpeg/恶意媒体容器、CPU/内存硬限额、目标低端设备 Argon2id 与 Fuzz 仍不能算完成 |
