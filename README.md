...
> [!IMPORTANT]
> Gallery 当前仍处于 pre-alpha 开发阶段。阶段 0～4 后端主线、阶段 5 安全代码基线及阶段 6 Web/PWA 页面代码基线均已实现；Chrome/Edge 已完成合成 smoke 与真实后端认证主路径验证，但阶段 5 Security Gate 和阶段 6 Web Gate 均未通过。2026-07-23 的独立审计（[验证记录 EV-39](Documents/证据/验证记录.md)）用真实浏览器复核发现：实时 WebSocket 通道在真实浏览器中无法建立，网页端多数写入口因权限名与后端不符而不渲染，因此网页界面当前在真实后端下接近只读。当前没有安装发行版本或完整使用教程，也尚未完成真实全量性能、SMB/NAS、真实 LAN 多设备、目标低端设备、Firefox/真实移动设备和跨平台发行门禁。
...
| 图形界面 | 响应式 Web/PWA 覆盖浏览、作品/媒体、Overlay、任务、规则、安全和维护页面 | 页面代码基线已实现；实时通道与多数写入口在真实后端下当前不可用，完整浏览器门禁未通过 |
...
| 实时通信 | 版本化 WebSocket（后端已实现；浏览器端握手缺陷待修复） |
...
| 阶段 6：Web/PWA 界面 | 🟠（页面代码基线已实现，但真实后端下多数写入口不可达） | 🟡（Chrome/Edge 认证主路径通过；实时通道与写路径 E2E 未覆盖，正式 Gate 未通过） | 同源内嵌 Web/PWA 已覆盖浏览、媒体、Overlay、任务、规则、安全和维护页面骨架 | 实时 WebSocket 在真实浏览器中握手失败；6 个权限名与后端不符导致 Overlay/任务/Library/治理写入口永不渲染；Firefox、真实移动设备/屏幕阅读器及完整写路径 E2E 未完成 | 先修复实时通道与权限名，再扩大浏览器与可访问性门禁，不进入桌面壳 |
