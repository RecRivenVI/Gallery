...
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口、设计重构与主要真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、媒体优先画廊端与紧凑管理端；EV-54～EV-91 覆盖主要业务/治理/恢复与弱网切片，EV-102 部分采纳共享动效基座，EV-113～EV-118 建立全路由/关键状态可访问性组合，EV-129/132 清除全部依赖审计例外，EV-136～EV-143 已窗口化主要大列表、安全资源与规则配置编辑器，并补齐 API 会话失效回退 | 浏览器业务门禁仍使用合成 Source；EV-92 只补到独立 testlab 的真实 Pixiv 有界预检。真实存储浏览器链、其余弱网矩阵、真实缩放/系统高对比、触摸设备与屏幕阅读器未验证；规则配置极限规模/深度仍未验证 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-91、EV-102、EV-113、EV-116～EV-118、EV-129、EV-132、EV-136～EV-143 | 🟡 | Vitest 15 个文件 223 项；Chromium/Firefox mock smoke 36/36；19 条路由在桌面/窄屏各 38 个状态通过 axe，并在 320px 模拟强制颜色/文本间距下覆盖路由及五个 mock 关键交互；精确生产资产上的 Chromium/Firefox 完整真实运行器各 23/23 仍是既有证据，full/production 依赖审计为 0 漏洞/0 例外；EV-136～EV-143 已限制主要大列表、安全资源与规则配置编辑器的当前页/视口 DOM，并补齐 HTTP 会话失效收敛 | 自动路由基线、作品身份交接、窄屏焦点、主要大列表和规则配置窗口及主要真实业务链已覆盖；真实移动/触控、人工屏幕阅读器、实际缩放/物理高对比、规则配置极限规模/深度、交互状态和弱网矩阵仍未完成 |
...
| 3 | 阶段 6 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检，EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消，EV-113/116 已建立当前全路由组合 axe，EV-117 已补五个关键交互状态，EV-118 又把其中的双入口配对与管理安全/维护状态接入真实后端，EV-129/132 已清除全部依赖审计例外，EV-136～EV-143 已限定主要大列表、安全资源与规则配置编辑器的单页/视口挂载规模并关闭 HTTP 会话失效缺口。下一步继续规则配置极限规模/深度、HDD/SMB/NAS、publishing/崩溃恢复、弱网/交互状态、人工辅助技术、真实缩放/系统高对比与真实设备 | Pixiv 全量扫描保持低优先级，不能替代业务闭环和发布可用性门禁 |
