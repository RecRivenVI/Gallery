...
| 阶段 4：查询与媒体 | 🟡（主线代码完成，部分参数未冻结） | 🟡（正确性已收口；500,000 publication 正式矩阵已完成 57/60、失败 0） | 搜索、排序、分页、显式规则/有效封面、媒体读取/下载、缩略图生成全部有代码闭环；EV-51/52 降低搜索与 publication 开销，EV-87/88 收窄聚合封面，EV-89/95 严格分页；EV-96～101 建立双关系、十目标来源、可续跑 publication 与 Query 正式入口，1%/10% 变化档各 20/20，50% 档已完成 17/20 并持续运行 | publication 的 50% 变化档及 Query 并发/冷缓存、Degradation 仍未完成；超大合并图、排序权重、Total、租约、兼容版本策略和正式 API Freeze 尚未冻结 | 完成正式 500k 性能矩阵并冻结接口 |
...
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口、设计重构与主要真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、媒体优先画廊端与紧凑管理端；EV-54～EV-91 覆盖主要业务/治理/恢复与弱网切片，EV-102 部分采纳共享动效基座，EV-113～EV-118 建立全路由/关键状态可访问性组合，EV-129/132 清除全部依赖审计例外，EV-136～EV-144 已窗口化主要大列表与规则配置，并让超出后端 256 层的结构树有界降级 | 浏览器业务门禁仍使用合成 Source；EV-92 只补到独立 testlab 的真实 Pixiv 有界预检。真实存储浏览器链、其余弱网矩阵、真实缩放/系统高对比、触摸设备与屏幕阅读器未验证；规则配置极限项数及完整草稿解析/内存仍未验证 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-91、EV-102、EV-113、EV-116～EV-118、EV-129、EV-132、EV-136～EV-144 | 🟡 | Vitest 15 个文件 225 项；Chromium/Firefox mock smoke 36/36；19 条路由在桌面/窄屏各 38 个状态通过 axe，并在 320px 模拟强制颜色/文本间距下覆盖路由及五个 mock 关键交互；精确生产资产上的 Chromium/Firefox 完整真实运行器各 23/23 仍是既有证据，full/production 依赖审计为 0 漏洞/0 例外；EV-136～EV-144 已限制主要大列表与规则配置挂载、对齐 256 层边界并补齐 HTTP 会话失效收敛 | 自动路由基线、作品身份交接、窄屏焦点、主要大列表、规则配置窗口/深度守卫及主要真实业务链已覆盖；真实移动/触控、人工屏幕阅读器、实际缩放/物理高对比、规则配置极限项数和完整草稿解析/内存、交互状态及弱网矩阵仍未完成 |
...
| 1 | 阶段 4 收尾 | 继续完成正在运行的 500k publication 50% 变化档，再分窗执行 Query 冷/热并发与 Degradation 性能门禁并完成 API 接口冻结 | 文档封印时已完成 57/60、失败 0；1%/10% 各 20/20，50% 为 17/20 并持续运行，正式多样本结果和接口数值仍未冻结；实时数值以 runner 报告为准 |
| 2 | 阶段 5 | 完成真实 LAN 多设备与目标低端设备 Argon2id 延迟/并发验证 | 同机 Chrome/Edge 和高性能工作站证据已取得，剩余缺口需要外部设备环境 |
| 3 | 阶段 6 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检，EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消，EV-113/116 已建立当前全路由组合 axe，EV-117 已补五个关键交互状态，EV-118 又把其中的双入口配对与管理安全/维护状态接入真实后端，EV-129/132 已清除全部依赖审计例外，EV-136～EV-144 已限定主要大列表与规则配置挂载规模、对齐 256 层容器边界并关闭 HTTP 会话失效缺口。下一步继续规则配置极限项数和完整草稿解析/内存、HDD/SMB/NAS、publishing/崩溃恢复、弱网/交互状态、人工辅助技术、真实缩放/系统高对比与真实设备 | Pixiv 全量扫描保持低优先级，不能替代业务闭环和发布可用性门禁 |
