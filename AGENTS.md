...
「真实后端 E2E 未进入 CI」仍是当前最大的门禁缺口之一。`gallery-rules.json` 所需的三级聚合封面、独立只读文件根、平台呈现/排序集下发均已实现；EV-50 已锁定 Source-local 聚合资源边界并消除 Creator→Source 的二次候选扇出，排序协议 v2 已贯通 title/name、publishedAt、Progress、签名 keyset 与 Web 规则菜单，但尚未取得 Reference Performance/API Freeze 证据，Creator/Library 聚合封面的逐主体授权裁剪也未完成。
