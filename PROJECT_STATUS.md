...
| 阶段 5：账户、安全与多客户端 | 🟠（代码与合成安全收尾已实现；恶意输入缺陷已收口） | 🟡（Personal 与同机 LAN 安全管理补证，正式 Gate 未通过） | EV-37/EV-38/EV-44/EV-48 的安全收口之外，EV-60 已把 Session、API Token、Share、allow/deny Grant、账户停用恢复和精确 Session 吊销接入隔离真实浏览器，并修正 global effective capability 误报 | 真实 LAN 多设备、目标低端设备 Argon2id、真实恶意资源和外部安全测试门禁仍未完成；同机 loopback 不能替代正式 Security Gate | 完成外部设备与恶意资源门禁 |
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口与首批真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、画廊端与管理端双入口；EV-54～EV-59 覆盖管理自举、同快照画廊/媒体、CustomCover、规则生命周期/ParameterSet 与 RJSF/AJV 表单，EV-60 覆盖安全管理和真实 WS 断线 snapshot 恢复，EV-61 覆盖备份、恢复验证/登记与 GC dry-run，EV-62 覆盖规则绑定状态、作品人工解绑/撤销和 retry-backoff Job 取消/同 ID 重试，EV-64 以同一隔离 AppDirs 实际重启证明 control 恢复生效，EV-65 把同一完整链扩展到桌面 Firefox，EV-66 关闭显式扫描被 Watcher 重复调度和维护终态不刷新任务表的缺口，EV-67～EV-70 补齐 15 类现有 primitive config 的权威 Schema 字段、当前草稿 Dry Run/Explain/Trace、按字段撤销及参数 Schema/tests/extensions 无损结构化编辑，EV-71 证明单帧 sequence gap 后会重取 HTTP 全快照，EV-72 证明运行中 Scan 取消会级联活动 Hash 且不发布半成品，EV-73 证明真实强杀后同 AppDirs 立即重启会接管未来租约下的遗留 Attempt 并支持 UI 治理，EV-74 建立首批治理链，EV-75 再覆盖 SourceWork merge、全部 orphan decision/实体类型与已消费决策冲突 | 门禁仍只用合成 Source；剩余治理组合、真实存储取消与崩溃恢复响应、真实 `MEDIA_READ_BUSY`、完整弱网抖动矩阵、触摸设备与屏幕阅读器未验证；画廊端无 DOM 虚拟化 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
| 阶段 7：平台适配与正式发行 | ⏳（仅早期实验代码涉及，不属于正式产品） | ⛔ | 无 | Windows 之外的平台、安装包、签名、升级等均尚未开始 | 最后阶段 |

**概览**：Gallery 已有正式后端和同源内嵌 Web/PWA 代码基线，Chrome/Edge 已验证主要认证与浏览器恢复路径，Chromium/Firefox 已通过隔离真实后端完整业务链；但真实大规模、真实多平台、真实网络硬盘、完整 Security/Web Gate 和正式发行仍未完成。2026-07-23 的独立审计（[验证记录 EV-39](Documents/证据/验证记录.md)）用真实浏览器探针发现阶段 6 的「业务闭环」此前被高估：实时 WebSocket 通道在真实浏览器中 100% 握手失败，网页端多数写入口因 capability 名不符而不渲染。这些阻断性缺陷已在同日的 [EV-40](Documents/证据/验证记录.md) 修复并经真实 Chrome/Edge 复验；[EV-54～EV-59](Documents/证据/验证记录.md) 随后把管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet、无损文本与 Schema 表单接入隔离真实后端持续门禁，[EV-60](Documents/证据/验证记录.md) 再加入安全资源管理、独立 loopback LAN 账户/Grant/Session 和真实 WebSocket 断线 snapshot 恢复，[EV-61](Documents/证据/验证记录.md) 覆盖 control 备份、恢复验证/登记与 Catalog GC dry-run，[EV-62](Documents/证据/验证记录.md) 再覆盖规则绑定状态、作品人工解绑/撤销与 retry-backoff Job 取消/同 ID 重试，[EV-64](Documents/证据/验证记录.md) 又以同一隔离 AppDirs 的实际重启证明 control 恢复生效，[EV-65](Documents/证据/验证记录.md) 再把相同 Personal/LAN 链扩展到桌面 Firefox，[EV-66](Documents/证据/验证记录.md) 随后关闭显式扫描与 Watcher 状态脱节、扫描期间事件可能丢失和维护 Job 终态不触发任务快照刷新的缺口，[EV-67](Documents/证据/验证记录.md) 把 15 类现有 primitive config 接入权威 Schema 可视化字段，[EV-68](Documents/证据/验证记录.md) 打通当前草稿 Dry Run/Explain/Trace，[EV-69](Documents/证据/验证记录.md) 建立以本地精确基线为准的按字段撤销，[EV-70](Documents/证据/验证记录.md) 再补齐参数 Schema、tests、extensions 的无损结构化编辑，[EV-71](Documents/证据/验证记录.md) 覆盖真实单帧 sequence gap，[EV-72](Documents/证据/验证记录.md) 覆盖运行中 Scan/Hash 级联取消，[EV-73](Documents/证据/验证记录.md) 覆盖进程强杀后的启动接管、recovered Attempt 与 UI 治理，[EV-74](Documents/证据/验证记录.md) 建立首批治理链，[EV-75](Documents/证据/验证记录.md) 再覆盖 SourceWork merge、全部 orphan decision/实体类型与已消费决策冲突。这些链路仍使用合成 Source；剩余治理组合和真实设备/可访问性仍未完成，阶段 6 Web Gate 依旧未通过。
...
| 阶段 6 业务闭环 | 「普通浏览器完成所有业务闭环后，才开始壳集成」 | **完成度被高估后已下调，现恢复首批持续切片** | EV-39 实测发现 WebSocket 与 capability 阻断；EV-40 修复，EV-54～EV-75 又以隔离真实后端覆盖管理自举、扫描 publication、WS→HTTP snapshot、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销与三个任意 JSON 根字段结构化编辑、安全、维护、规则绑定状态、作品/媒体人工解绑与撤销、Binding issue、SourceWork split/merge、全部 orphan decision/实体类型、已消费决策冲突、retry-backoff Job、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启，并修复显式扫描/Watcher 与维护终态实时失效竞态 | EV-39/40 真实 Chrome/Edge 探针；EV-54～EV-75 Chromium/Firefox 真实 `galleryd` 合成 Source 持续 E2E | 已覆盖规则、安全、维护、扩展治理/任务状态链、运行中取消、强杀启动接管与 control 恢复重启，但完整业务闭环条件仍未满足，不得进入壳集成 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| 浏览/搜索/作品/作者/媒体、Overlay、任务、规则、安全与维护界面 | 需要 | 以 OpenAPI/HTTP snapshot 为事实源，同源嵌入 `galleryd`；列表/详情/封面/媒体沿用同一 publication | `web/src/pages/*`、`web/src/components/work-cover.tsx`、`web/src/manage/rules/*`、`web/src/api/*`、`internal/webapp` | 🟠（页面骨架已实现；EV-40 已关闭 capability 写入口阻断，EV-42 新增实际封面与 CustomCover 编辑，EV-55～EV-59 补齐真实媒体/CustomCover/规则生命周期/ParameterSet/Schema 表单，EV-60 补齐安全管理首链，EV-61 补齐维护首链，EV-62 补齐规则绑定状态和首条治理/任务状态链，EV-64 补齐 control 恢复实际重启，EV-65 补齐桌面 Firefox 完整链，EV-66 补齐扫描/Watcher 与维护 Job 实时收敛，EV-67～EV-70 补齐现有 primitive 字段、当前草稿 Dry Run/Explain/Trace、按字段撤销与三个任意 JSON 根字段结构化编辑，EV-71 补齐真实单帧 sequence gap 的全快照恢复，EV-72 补齐运行中 Scan/Hash 级联取消，EV-73 补齐进程强杀后的启动接管、recovered Attempt 与 UI 治理，EV-74 建立首批治理链，EV-75 补齐 SourceWork merge、全部 orphan decision/实体类型及已消费冲突） | Vitest、Playwright mock、真实后端 Chrome/Edge Personal/LAN 与 Chromium/Firefox 完整链 | EV-38～EV-40、EV-42、EV-54～EV-75、ADR-009 | 🟡 | 合成画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿调试、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全、维护、扩展治理、retry-backoff Job、运行中 Scan/Hash 级联取消、进程强杀启动接管和 control 恢复实际重启已进入 Chromium/Firefox 真实后端 E2E；剩余治理组合、真实移动设备与跨设备 LAN 仍未覆盖 |
| 实时通道（WebSocket → HTTP snapshot 恢复） | 需要 | WS 只作提示，断线/gap 以 HTTP snapshot 恢复 | `web/src/shared/realtime.tsx`、`internal/contract/realtime` | ✅（EV-40 修复 `WS-1`/`WS-2`，EV-71 补齐单帧 gap 恢复） | 真实 Chrome/Edge 连接/事件/CSP；隔离 Chromium/Firefox 真实断线与单帧 gap 后 snapshot 恢复 | EV-39、EV-40、EV-60、EV-71 | 🟡 | EV-60 已覆盖真实 socket 断开、离线期间另一 Session 改变事实与重连收敛；EV-71 已覆盖 Chromium/Firefox 同一连接的单帧 sequence gap；完整弱网抖动矩阵仍未覆盖 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-75 | 🟡 | Vitest 14 个文件 194 项；Chromium/Firefox mock smoke 8/8；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium 与 Firefox/真实 `galleryd` 整条运行器各执行 16 个实际测试、同 AppDirs 恢复重启与六个治理子根 Source guard | 管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全、断线与单帧 sequence gap 恢复、维护、扩展治理、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启已用 Chromium/Firefox 纳入 CI workflow；EV-66 另以两浏览器全链锁定扫描/Watcher 与维护终态同步；剩余治理组合、真实移动设备/屏幕阅读器仍未覆盖 |
...
| WebSocket 实时通道 | `/ws/v1` 推送任务、publication、安全吊销等事件 | EV-40 修复握手与信封字段后，真实 Chrome/Edge 已验证连接建立并收到事件；EV-54 又在真实后端 E2E 中以新 Job、列表 refetch 与页面 sequence 严格递增断言 `job.*` 事件后的 HTTP snapshot 恢复；EV-60 覆盖断线重连，EV-71 覆盖 Chromium/Firefox 的单帧 sequence gap 与全 snapshot 重取。其他事件类型和完整弱网矩阵仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-75 又覆盖管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全、断线与单帧 sequence gap 的 snapshot 恢复、维护、规则绑定状态、扩展治理、retry-backoff Job、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启，并让完整链在桌面 Firefox 通过、关闭扫描/Watcher 和维护终态同步竞态。但真实移动设备/屏幕阅读器、窄屏侧栏焦点陷阱、剩余治理组合与正式可用性 Gate 均未完成 |
| 测试体系缺口 | EV-54～EV-75 已让管理自举、画廊/媒体、CustomCover、规则 lifecycle/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全资源管理、断线与单帧 sequence gap 恢复、维护、扩展治理/任务状态链、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启的 Chromium/Firefox 真实后端 E2E 进入 CI workflow，并保持 mock smoke 与真实证据分层；EV-45 已让阶段 4 testlab Correctness 进入普通 `go test`，但 500k 性能/十来源矩阵仍是人工门禁；全仓库仍无正式 Fuzz，部分平台包仍缺直接测试。完整弱网抖动矩阵、剩余治理组合、真实存储崩溃恢复等路径仍未进入真实后端持续门禁 |
...
| 3 | 阶段 6 | 在 EV-54～EV-75 的 Chromium/Firefox 管理自举、画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全、断线与单帧 sequence gap 恢复、维护、扩展治理/任务状态、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启门禁上，继续覆盖剩余治理组合、完整弱网抖动矩阵、可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
