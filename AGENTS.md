...
- 尚未完成：阶段 5 真实物理 LAN 多设备、目标低端设备 Argon2id 及非 Windows 外部工具进程树资源门禁；阶段 6 真实移动设备/触控、人工屏幕阅读器、真实浏览器 200%/400% 缩放、物理操作系统高对比与交互状态组合的完整可访问性、8 MiB 内规则完整草稿解析/序列化/AJV/内存极限及无 Schema 项数上限对象的资源响应、任意规则的完整可视化构建、真实存储上的取消与崩溃恢复响应及完整弱网抖动矩阵；以及 Pixiv/真实 HDD 全量扫描与哈希、SMB/NAS/UNC、Linux 原生与重挂载/跨卷文件身份稳定性、真实慢盘维护中间进度/VACUUM 取消/磁盘满、正式 Query 分类/候选量阈值、结构化过滤计划与 Degradation Performance Gate、ranking/total/租约及外部工具资源预算等 PRE_FREEZE 数值、AND/OR canonical 化，以及正式 Authenticode、安装更新、更多升级跨度、恢复磁盘满、低完整性/多账户/继承 ACL、其它指令窗口强杀与真实断电及平台发行。
