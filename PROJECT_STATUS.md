...
| 3 | 阶段 6 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检、discovery 取消与同 AppDirs 恢复重跑；EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消切片。下一步继续 HDD/SMB/NAS、publishing/崩溃恢复、其余弱网矩阵、全页面可访问性与真实设备；同时固定全量运行根、空间、续跑/失败恢复和监控报告方案 | Pixiv 全量扫描保持低优先级，不能替代业务闭环和发布可用性门禁 |
