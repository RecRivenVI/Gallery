...
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携测试基线、同源切换及损坏备份回滚主路径） | 🟡（本地制品、切换/恢复及损坏备份 smoke 通过，正式 Gate 未通过） | 精确干净提交可生成同源双前端 ZIP、三份 SBOM、清单、摘要与签名状态门禁；两个独立版本标签已在同 AppDirs 完成用户事实承接、备份校验、重启恢复及损坏备份保留当前库 | 正式签名、安装/更新、真实历史升级/降级、磁盘满/权限/中断回滚、平台矩阵及桌面壳未完成 | 完成 Windows RC 门禁 |
...
EV-105 将损坏备份失败回滚加入同一真实制品链：正常恢复后再次建立备份与备份后用户事实，登记恢复、优雅停机，再只破坏临时 AppDirs 中的专用备份；下一次 `galleryd` 启动保留当前库和备份后事实，消费 pending 标记并记录失败，程序目录仍零变化。精确干净提交 `61211f2` 的 `0.1.9-ev105` 与 `0.2.0-ev105` 本地包均为 `dirty=false`、`unsigned` 并通过。历史 Schema 升级/降级、磁盘满、权限失败、原子落位中断、正式签名和 RC 仍未完成。
