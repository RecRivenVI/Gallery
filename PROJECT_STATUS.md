...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| 浏览/搜索/作品/作者/媒体、Overlay、任务、规则、安全与维护界面 | 需要 | 以 OpenAPI/HTTP snapshot 为事实源，同源嵌入 `galleryd`；列表/详情/封面/媒体沿用同一 publication | `web/src/gallery/pages/*`、`web/src/gallery/components/*`、`web/src/manage/pages/*`、`web/src/manage/rules/*`、`web/src/api/*`、`internal/webapp` | 🟠（页面代码基线与 EV-54～EV-77 的真实后端业务/治理持续链已建立；EV-78 新增双入口窄屏模态导航和当前页语义） | Vitest、Playwright mock、真实后端 Chrome/Edge Personal/LAN 与 Chromium/Firefox 完整链 | EV-38～EV-40、EV-42、EV-54～EV-78、ADR-009 | 🟡 | 主要合成业务闭环已进入 Chromium/Firefox 真实后端 E2E；真实移动设备、人工屏幕阅读器与跨设备 LAN 仍未覆盖 |
...
| 无障碍访问（键盘、屏幕阅读器等） | 需要 | React Aria + 语义 token、reduced motion、焦点与触控样式 | `web/src/design/*`、`web/src/gallery/components/chrome.tsx`、`web/src/manage/layout.tsx`、`web/e2e/gallery.spec.ts` | 🟠（EV-78 已关闭当前 UI 的窄屏导航焦点边界） | Chromium/Firefox Playwright + axe：桌面双入口和 390×844 双模态导航，`color-contrast` 保持启用 | EV-38～EV-40、EV-78 | 🟡 | 已覆盖常驻/模态导航互斥、正反向 Tab、Escape、焦点返还、路由关闭、`aria-current` 与无横向溢出；仍缺全页面审计、人工屏幕阅读器、真实触控/虚拟键盘/安全区和缩放门禁 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-78 | 🟡 | Vitest 14 个文件 194 项；Chromium/Firefox mock smoke 10/10；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium 与 Firefox/真实 `galleryd` 整条运行器各执行 17 个实际测试、同 AppDirs 多阶段重启与 11 个治理子根 Source guard | 真实后端持续链覆盖管理自举、画廊/媒体、规则、安全、维护、治理、取消和重启恢复；mock smoke 另覆盖双入口桌面/390×844 导航与 axe。真实移动设备/屏幕阅读器仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-77 覆盖主要真实后端业务与治理持续链，EV-78 又关闭当前双入口 UI 的窄屏导航焦点边界并纠正旧侧栏状态漂移。但真实移动设备/触控、人工屏幕阅读器、全页面可访问性与正式可用性 Gate 均未完成 |
...
| 3 | 阶段 6 | 在 EV-54～EV-77 的 Chromium/Firefox 真实后端业务/治理链和 EV-78 双入口窄屏导航门禁上，继续覆盖完整弱网抖动矩阵、全页面可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
