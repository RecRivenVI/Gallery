...
| 阶段 6 业务闭环 | 「普通浏览器完成所有业务闭环后，才开始壳集成」 | **完成度被高估后已下调，现恢复首批持续切片** | EV-39 实测发现 WebSocket 与 capability 阻断；EV-40 修复，EV-54～EV-62 又以隔离真实后端覆盖管理自举、扫描 publication、WS→HTTP snapshot、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、维护、规则绑定状态、作品人工解绑/撤销与 retry-backoff Job | EV-39/40 真实 Chrome/Edge 探针；EV-54～EV-62 Chromium/真实 `galleryd` 合成 Source 持续 E2E | 已覆盖规则、安全、维护与首条治理/任务状态链，但任意规则的完整可视化构建及完整业务闭环条件仍未满足，不得进入壳集成 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| 浏览/搜索/作品/作者/媒体、Overlay、任务、规则、安全与维护界面 | 需要 | 以 OpenAPI/HTTP snapshot 为事实源，同源嵌入 `galleryd`；列表/详情/封面/媒体沿用同一 publication | `web/src/pages/*`、`web/src/components/work-cover.tsx`、`web/src/manage/rules/*`、`web/src/api/*`、`internal/webapp` | 🟠（页面骨架已实现；EV-40 已关闭 capability 写入口阻断，EV-42 新增实际封面与 CustomCover 编辑，EV-55～EV-59 补齐真实媒体/CustomCover/规则生命周期/ParameterSet/Schema 表单，EV-60 补齐安全管理首链，EV-61 补齐维护首链，EV-62 补齐规则绑定状态和首条治理/任务状态链） | Vitest、Playwright mock、真实后端 Chrome/Edge Personal/LAN | EV-38～EV-40、EV-42、EV-54～EV-62、ADR-009 | 🟡 | 合成画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、维护、作品人工解绑/撤销和 retry-backoff Job 已进入真实后端 E2E；任意规则完整可视化、其余治理、长任务取消/重启恢复、Firefox、真实移动设备与跨设备 LAN 仍未覆盖 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-62 | 🟡 | Vitest 14 个文件 180 项；Chromium mock smoke 4/4；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium/真实 `galleryd` 为 Personal 10/10、LAN 1/1 与 Source guard | 管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、断线恢复、维护与首条治理/任务状态链已纳入 CI workflow；任意规则完整可视化、其余治理、长任务取消/重启、Firefox/真实移动设备/屏幕阅读器仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-62 又覆盖管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、断线 snapshot 恢复、维护、规则绑定状态、作品人工解绑/撤销与 retry-backoff Job。但 Firefox、真实移动设备/屏幕阅读器、窄屏侧栏焦点陷阱、任意规则的完整可视化构建、其余治理路径、长任务取消/重启恢复与正式可用性 Gate 均未完成 |
| 测试体系缺口 | EV-54～EV-62 已让管理自举、画廊/媒体、CustomCover、规则 lifecycle/ParameterSet/Schema 表单、安全资源管理、断线恢复、维护与首条治理/任务状态链的 Chromium/真实后端 E2E 进入 CI workflow，并保持 mock smoke 与真实证据分层；EV-45 已让阶段 4 testlab Correctness 进入普通 `go test`，但 500k 性能/十来源矩阵仍是人工门禁；全仓库仍无正式 Fuzz，部分平台包仍缺直接测试。显式 WS sequence gap/弱网抖动、任意规则完整可视化、其余治理、长任务取消及真正的重启恢复等路径仍未进入真实后端持续门禁 |
...
| 3 | 阶段 6 | 在 EV-54～EV-62 的管理自举、画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、断线恢复、维护与首条治理/任务状态门禁上，继续覆盖任意规则的完整可视化构建、其余治理、长任务运行中取消与真正的重启恢复、显式 WS sequence gap/弱网抖动、浏览器/可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
