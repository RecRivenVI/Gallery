...
- Pixiv 全量只读扫描是候选 RC 完成其它开发与门禁后的最后一道验收门槛，不是可跳过的低优先级验证；必须以候选 RC 完成完整规则、扫描/哈希/发布、前端可用性和 Source 零写入验收，通过后才可交付 RC 实测。
- 当前仓库已有正式产品代码（`cmd/`、`internal/`、`pkg/`、`web/`）。阶段 0 契约骨架、Walking Skeleton、Architecture Proof、阶段 1「领域和数据所有权」、阶段 2「规则闭环」、阶段 3「扫描、任务和 Catalog」与阶段 4「查询和媒体」均已完成代码与合成 Correctness；阶段 5 已完成账户/安全/多客户端代码与合成收尾，并取得同机 Personal/LAN 与当前工作站 Argon2id 补证，但正式 Security Gate 未通过；阶段 6 已实现 React/TypeScript Web/PWA 双入口及认证、浏览/媒体、Overlay、任务、规则、安全和维护页面，EV-54～EV-91 已将主要业务、治理、恢复、弱网切片、响应式、Source 作者与授权 Job 历史接入 Chromium/Firefox 真实 `galleryd` 门禁，EV-102 又部分采纳共享动效基座，补齐同范围旧快照保护、作品身份承接、媒体显现、直接操纵、浮层与管理端克制反馈，但正式 Web Gate 未通过。EV-92 首次把 legacy schema v3 的真实 Pixiv 规则经正式转换接入独立 AppDirs，在 370,712 文件/105,202 目录上完成 45 秒有界 `index` 与全树零写入 guard；EV-93 修复 discovery 不响应 Scan context，EV-94 再以真实 Pixiv 将扫描总耗时从修复前 116,584 ms 收敛到 45,397 ms，并在取消 POST 后 201 ms 观察到终态、同 AppDirs 恢复重跑 7/7 通过。该结论只覆盖 Windows 本地 SSD/Pixiv/index discovery，不是全量扫描/哈希、完整规则语义或性能 Gate。阶段 4 的 EV-35/36/51/52/87～89 已建立 500,000 合成 Correctness 与多项候选/聚合性能基线，EV-95 又关闭 Creator 治理入口的无界响应；EV-96～101 建立十来源、双 Creator 关系、全局 1%/10%/50% 变化的 production publication 与 Query Reference 执行器及 fail-closed 断点；EV-147 已完成 publication 60/60；EV-150～154 又依次收口宽标题前缀、重复 Total、查询 P95 profile/生产结构化计划、重复在途页计算与冷进程首次 Total 成本。在同一 500,000 Work、十来源、双关系 Catalog 上，最终 `query-reference-500k-p95-v1` 的 warm/cold-process 各 63/63、130 findings、失败和超时均为 0，Query Reference 子矩阵通过。cold-process 不清空 OS 文件缓存，并发结果只代表相同查询页风暴；混合查询吞吐、冷存储、维护、完整哈希与 Degradation 仍未完成，`TotalBudget=5000` 也仍为 PRE_FREEZE 候选，因此完整 Reference/Degradation Performance Gate 与 API Freeze 仍未通过。阶段 7 的 EV-103～EV-107 已建立未签名 Windows 便携制品、同源切换/恢复、损坏备份失败回滚、真实 schema 23→24/反向拒绝与首次轮换拒绝；EV-109/110/119 又覆盖落位后连续性 fail-closed、真实安全回滚和包内双 sharing-violation，EV-121～125 再关闭当前库缺失、落位后安全收尾、状态文件失败、同进程双 Rename 与精确强杀窗口；EV-133 又把 Windows v1 已验证升级范围连续扩展为 control schema 20/21/22/23→24。正式签名、安装/更新、schema 20 以前的开发快照、磁盘满/低完整性/多账户/继承 ACL、其它恢复窗口强杀与真实断电仍未完成。EV-108/111 已用真实 schema v3 转换结果关闭 Gank/Pawchive“完全未进入真实 Source”的旧缺口并消除 Pawchive 12 目标重复全 Source 处理，但完整真实规则语义、Pixiv/真实盘全量扫描与哈希仍未完成。
...
- [EV-137](Documents/证据/验证记录.md)（2026-07-30）在不改变 publication-bound keyset 连续加载的前提下，把用户画廊作品按固定 48 项块窗口化：视口附近挂载真实卡片，远端块以实测高度占位，宽度变化使有界高度缓存失效；路由在旧 DOM 替换前保存并恢复历史滚动位置，末页加载不再留下悬空 live-announcer。576 项双浏览器 mock 定向、28/28 完整 mock、Chromium/Firefox 各 23/23 完整真实后端链及 819.8 秒根级检查通过。**该切片只限定已加载作品的 DOM 规模，不限制查询缓存，也不代表真实 500k UI、真实设备、人工辅助技术或完整 Web Gate。**
- [EV-138](Documents/证据/验证记录.md)（2026-07-30）把 Binding issue 与 orphan candidate 两条既有授权后 keyset 管理列表从跨页 `flatMap` 收敛为每页最多 50 条的当前页窗口：前后导航复用已访问页，续页失败保留当前 HTTP snapshot 并原地重试，筛选重置第一页；Job 分页的加载反馈也改为稳定可见文本。管理端 46/46 组件测试、Chromium/Firefox 治理定向各 2/2、完整 mock 28/28、精确生产资产完整真实后端链各 23/23 及 1011 秒根级检查通过。**该切片只限定两条治理表的单页 DOM 规模，不限制查询缓存，也不代表其它结构决策/安全资源列表、配置数组、真实设备或完整 Web Gate。**
- [EV-139](Documents/证据/验证记录.md)（2026-07-31）把用户端 Creator 与实时文件目录从跨页 `flatMap` 收敛为当前页/当前批窗口：Creator 每页最多 48 项并保留 Source/排序/keyset 语义，文件目录每批最多 500 项并明确不是可重复读快照；前后导航复用已访问页，续页失败保留当前窗口。双浏览器分页定向 4/4、完整 mock 30/30、最终精确生产资产完整真实后端链各 23/23 及根级检查通过。**该切片不限制查询缓存，不把实时目录变成 publication，也不代表真实超大目录、真实设备或完整 Web Gate。**
- [EV-140](Documents/证据/验证记录.md)（2026-07-31）修复 Firefox 丢失 WebSocket 4401 时 Session 吊销不能及时撤下旧认证壳的缺口：任一查询或变更返回 `UNAUTHENTICATED`/`CSRF_INVALID` 时，`SessionProvider` 立即撤下旧主体子树、清除非 bootstrap 缓存并重新拉取 bootstrap，且以单一 in-flight 任务防止收敛风暴。会话/实时组件 30/30、Chromium/Firefox 精确生产资产完整真实后端链各 23/23、mock 30/30 与根级 217 项 Vitest/全仓门禁通过。**该切片不代表物理多设备 LAN、完整弱网、真实移动/辅助技术或完整 Security/Web Gate。**
- [EV-141](Documents/证据/验证记录.md)（2026-07-31）把结构决策历史从“新到旧最多 50 条且不可续页”改为 `(created_at, decision_id)` keyset 分页，并以 control v25 的全局、Source、Source + status 三条索引支撑读取；OpenAPI/生成客户端同步 `cursor`/`nextCursor`，管理端只渲染当前最多 50 条并支持失败重试与缓存往返。55 条应用层三页、HTTP 续页/非法 cursor、管理组件 47/47、双浏览器 51 条 mock 定向 2/2、完整 mock 32/32 与 1,228.7 秒根级 218 项 Vitest/全仓门禁通过。**该切片没有单独建立 51 条真实 `galleryd` 浏览器专项，不限制查询缓存，也不代表安全资源等其它管理列表、配置数组、真实设备或完整 Web Gate。**
- [EV-142](Documents/证据/验证记录.md)（2026-07-31）为 Session、API Token、Share、本地账户与 Grant 增加默认 50、上限 200 的 live keyset 页面，cursor 严格绑定资源类型，并在存在主体过滤时绑定 Principal/目标主体，由 control v26 五条索引支撑；OpenAPI/生成客户端同步 `limit`/`cursor`/`nextCursor`，管理端五张表只挂载当前页，支持失败保留、重试和缓存往返，安全写入或实时变化后重置旧分页族。五类各 55 条应用层三页、HTTP 越界/跨作用域 cursor、管理组件 48/48、双浏览器 51 条 mock 定向 2/2、完整 mock 34/34 与 1,283.1 秒根级 219 项 Vitest/全仓门禁通过。**该切片没有重跑同规模真实 `galleryd` 浏览器专项，不限制查询缓存，也不代表配置数组、真实设备或完整 Security/Web Gate。**
- [EV-143](Documents/证据/验证记录.md)（2026-07-31）为规则编辑器建立每页最多 20 项的本地挂载窗口：所有 RJSF 数组、参数 Schema 属性、tests、extensions、递归 JSON 对象/数组与字段撤销列表只挂载当前页，完整无损草稿、顺序、全局索引和服务端权威均不改变；同轮修复 CSS `:read-only` 误命中可编辑 `select` 而将对比度降到 4:1 的缺口。管理组件 52/52、Chromium/Firefox 定向 2/2、完整 mock 36/36 与 1,055.4 秒根级 223 项 Vitest/全仓门禁通过。**该切片只限制 DOM/编辑器子树，不限制完整本地草稿或浏览器内存；21 项 mock 边界不等于已验证 4,096/10,000 上限、恶意超深 JSON、真实后端同规模、真实设备或完整 Web Gate。**
- [EV-144](Documents/证据/验证记录.md)（2026-07-31）让规则结构化 JSON 编辑器在挂载前以显式栈核对完整 RulePackage JSON Pointer 深度，超过后端权威 256 层容器上限时暂停递归树并保留无损原始 JSON；Go 跨语言测试锁定前后端常量。同轮修复 Modal/Popover 祖先 opacity 造成的进入态瞬时 3.54:1 对比度：遮罩只过渡背景，内容只过渡位移。管理组件 54/54、15 文件 225 项 Vitest、双浏览器深度/高对比定向各 2/2、最终 mock 36/36 与 1,068.1 秒根级全仓门禁通过。**浏览器夹具只证明完整规则 257 层被有界降级；它不限制完整草稿文本、解析或内存，也不代表 4,096/10,000 项极限数组、任意解析器极限、同规模真实后端、真实设备或完整 Web Gate。**
- [EV-145](Documents/证据/验证记录.md)（2026-07-31）把规则 Schema 的正式数组上限直接纳入前端持续门禁：同一草稿包含 4,096 个 primitive 与 10,000 个 test 时，两类结构化编辑器各只挂载当前 20 项，页数分别为 205/500，切回无损 JSON 文本后末项仍完整存在。定向组件 1/1、Chromium/Firefox 2/2、全量 15 文件 226 项 Vitest、最终 mock 36/36 与 1,087.5 秒根级全仓门禁通过；测试提交 `ff960122159591fb48c9fc64c420bc8cf2e7ed08` 含原始 SSH `gpgsig`。**该切片不限制完整草稿文本、解析、序列化、AJV 或浏览器内存，不覆盖没有 Schema 项数上限的任意对象，也没有重跑同规模真实 `galleryd`、真实设备或完整 Web Gate。**
- [EV-146](Documents/证据/验证记录.md)（2026-07-31）修复规则内容的正式 8 MiB 契约被 HTTP 通用 1 MiB 解码上限提前截断的问题：普通 JSON API 继续保持 1 MiB，只有规则端点使用覆盖双精确文档最坏 JSON 转义的独立传输预算，每份内容仍由应用层按 8 MiB 拒绝；管理端按 UTF-8 字节计数，超限时保留原文但不进入 Lossless JSON、Schema/AJV 或保存链，Go 跨语言测试锁定前后端常量。真实 HTTP 测试覆盖大于 1 MiB 的 import/save 与大于 8 MiB 的应用层拒绝；组件 2/2、Chromium/Firefox 生产资产定向 4/4、全量 16 文件 228 项 Vitest、mock 38/38、Windows 受影响包、WSL2 race 与 1,385.1 秒根级全仓门禁通过；实现提交 `e9af9ab75e95267316277750e8e00e9d21867499` 含原始 SSH `gpgsig`。**该切片没有以真实 `galleryd` 浏览器提交同一超大正文，不限制前端保留原文的内存，也不代表 8 MiB 内最坏形状、无 Schema 项数上限对象、真实设备或完整 Web Gate。**
- [EV-147](Documents/证据/验证记录.md)（2026-07-31）完成阶段 4 的 500,000 Work/十权威目标来源/每 Work 两条 Creator 关系 publication 正式多样本矩阵：全局 1%/10%/50% 变化各 20/20，总计 60/60、失败/超时/未尝试均为 0，全部旧 active publication 跨构建保持可读；Publish P95 分别为 2.168/2.147/5.581 ms，均低于 250 ms 目标。36 小时低资源运行报告 SHA-256 为 `3121B7E3D3712DFE8F3460487D4954D3BCE78EE733F4A23A5FC81CE08145BE0A`。**该结果只通过 publication 子矩阵；报告为 warm cache 合成完整 Catalog，不代表真实 HDD/SMB/NAS，500,000 Query warm/cold-process 并发与 Degradation 仍未执行，完整 Reference Performance Gate/API Freeze 未通过。**
- [EV-148](Documents/证据/验证记录.md)（2026-07-31）把 EV-146 的分层证据闭合为同一个真实浏览器链：Chromium/Firefox 从可见管理端创建服务端生成 RuleSet ID 的独立规则包，填入大于通用 1 MiB、低于正式 8 MiB 的合成 JSON 草稿，经真实 `PUT`/`If-Match` 完整保存，再以页面重载后的 SHA-256 证明服务端持久文本无损；运行器新增互斥 `rule-size-only` 模式并把该 spec 加入完整链。独立专项各 1/1，最终完整链各 24/24，Source guard、强杀/恢复、长停机与 LAN 等价通过；实现提交 `871b11df1fcff604679274703e46c2a7a68c1b0d` 与时序修正 `47ed532f3e34f0378027d34267b31e2866f46868` 均含原始 SSH `gpgsig`。**这关闭真实浏览器大于 1 MiB 保存/重载缺口，但不是 8 MiB 内最坏结构、浏览器内存硬上限、真实设备或完整 Web Gate。**
- [EV-149](Documents/证据/验证记录.md)（2026-07-31）把规则 8 MiB 边界从输入扩展到 Import/Compile 规范化后的 canonical 输出，禁止 YAML/TOML 转换、默认值或 hash 物化生成服务自身无法再次接受的超限正文；真实 Chromium/Firefox 专项从 8 MiB−1 的可保存无效草稿恢复到可验证正文并重载无损。实现提交 `7d34a30b11e17e5701e02836996ba2dd454a5a29` 含原始 `gpgsig`；Windows/WSL2 定向、532.2 秒根级检查与双浏览器专项通过，未重跑完整 24/24 链。
- [EV-150](Documents/证据/验证记录.md)（2026-07-31）为可证明等价的全授权 title asc/desc 非数字结尾前缀查询增加 NaturalSortKey v2 有界范围快路径，并用每 publication 最多两份、只保留随机 64-bit hash 的精确值索引保护 Filename/Creator/Tag 更高档位语义；不确定、过滤、部分授权及数字结尾形态全部回退原路径。实现提交 `79246fa839254a041b204fa8200abb679ba25e68` 含原始 `gpgsig`；同形状 500,000 warm 矩阵的九个 `wide-cjk` 组合从旧超时收敛为全部通过，但当轮仍被 filename/structured 高并发拖到 42/63 后超时，完整 Query Gate 当时未过。
- [EV-151](Documents/证据/验证记录.md)（2026-07-31）以最多 256 条的硬上限缓存并合并同一不可变 publication、授权 Source 集合和查询语义的并发 Total；错误/取消不缓存，溢出的在途新键直接无缓存计算，`creator.id` 因依赖当前 control.db 等价组明确绕过。实现提交 `1a21bec2fd0d435aa9bcab1f010e3995232d7273` 含原始 `gpgsig`。同一 500,000 Work/十来源/双关系 Catalog 的正式 warm 与 cold-process 全矩阵均为 63/63、失败/超时 0；最慢 `filename-infix` limit 200/concurrency 16 的 warm P95/P99/最大为 22.807/26.264/29.357 秒，cold-process 为 23.501/27.486/28.931 秒。**这完成当前 Query 矩阵执行，但类别/候选量 P95 阈值、结构化过滤 EXPLAIN、冷存储和 Degradation 尚未冻结或完成，完整 Reference Gate/API Freeze 仍未通过。**
- [EV-152](Documents/证据/验证记录.md)（2026-07-31）在正式复跑前冻结 `query-reference-500k-p95-v1`：63 个组合逐项绑定类别、limit、确定性候选量、原文复核与 concurrency 预算，并把失败/超时、样本完整性和 P95 纳入 finding；生产 SQL builder 的 structured-and/or/overlay-favorite `EXPLAIN QUERY PLAN` 持续断言禁止 `SCAN w` 与临时排序。实现提交 `121d7a04513252738697ae644308296a2eb62c1e` 含原始 `gpgsig`；首轮正式 warm 真实暴露 filename limit 200/concurrency 16 的 1/100 超时并判 3 项失败，未放宽阈值或隐藏失败。
- [EV-153](Documents/证据/验证记录.md)（2026-07-31）为 production Query 页读增加最多 256 个键的纯在途合并：键由精确 SQL 与实参构成，结果深拷贝；首 waiter 取消不影响其它请求，最后 waiter 取消才停止 SQL，完成后不保留页面缓存。实现提交 `ddc7363445022e8e5ddae6ee21388db9620f508a` 含原始 `gpgsig`；正式 warm 63/63、130 findings 全绿，但 cold-process 的 structured-and limit 20/concurrency 4、16 分别以 2,269.533/2,652.247 ms 超出 1,750/2,500 ms，证明首次 Total 成本仍须处理。
- [EV-154](Documents/证据/验证记录.md)（2026-07-31）把 Total 扫描候选预算从 10,000 收紧为 5,000，并将实际 `TotalBudget` 与 `TotalProtocolVersion=1` 纳入 Query 矩阵身份；预算仍为 PRE_FREEZE 候选，不改变 total 三态协议。实现提交 `fb78402ffd58a1f0a22429f352fe54eb282ebbeb` 含原始 `gpgsig`。最终正式 warm/cold-process 各 63/63、130 findings、失败/超时 0；warm/cold 报告 SHA-256 分别为 `B75531E3F10E56E3B10BCEE14AC4F0768E8D70DC08080E79D60D8C386DE9F894` 与 `6856900EAA22CDC1E4DFD8827D952EB2C0F3B71400B7795DDCBECFEB53FCD64F`。**Query Reference 子矩阵通过；cold-process 不等于冷存储，相同页合并不代表混合查询吞吐，维护、完整哈希与 Degradation 仍未完成，因此完整 Reference Gate/API Freeze 仍未通过。**
...
- 尚未完成：阶段 5 真实物理 LAN 多设备、目标低端设备 Argon2id 及非 Windows 外部工具进程树资源门禁；阶段 6 真实移动设备/触控、人工屏幕阅读器、真实浏览器 200%/400% 缩放、物理操作系统高对比与交互状态组合的完整可访问性、8 MiB 内规则完整草稿解析/序列化/AJV/内存极限及无 Schema 项数上限对象的资源响应、任意规则的完整可视化构建、真实存储上的取消与崩溃恢复响应及完整弱网抖动矩阵；以及候选 RC 最后一道强制验收所需的 Pixiv 全量只读扫描/哈希/发布/前端可用性与 Source 零写入、真实 HDD 全量扫描与哈希、SMB/NAS/UNC、Linux 原生与重挂载/跨卷文件身份稳定性、真实慢盘维护中间进度/VACUUM 取消/磁盘满、混合查询吞吐/冷存储与 Degradation Performance Gate、ranking/total/租约及外部工具资源预算等 PRE_FREEZE 数值、AND/OR canonical 化，以及正式 Authenticode、安装更新、更多升级跨度、恢复磁盘满、低完整性/多账户/继承 ACL、其它指令窗口强杀与真实断电及平台发行。
