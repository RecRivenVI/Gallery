...
| 阶段 6 业务闭环 | 「普通浏览器完成所有业务闭环后，才开始壳集成」 | **完成度被高估后已下调，现恢复首批持续切片** | EV-39 实测发现 WebSocket 与 capability 阻断；EV-40 修复，EV-54～EV-73 又以隔离真实后端覆盖管理自举、扫描 publication、WS→HTTP snapshot、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销与三个任意 JSON 根字段结构化编辑、安全、维护、规则绑定状态、作品人工解绑/撤销、retry-backoff Job、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启，并修复显式扫描/Watcher 与维护终态实时失效竞态 | EV-39/40 真实 Chrome/Edge 探针；EV-54～EV-73 Chromium/Firefox 真实 `galleryd` 合成 Source 持续 E2E | 已覆盖规则、安全、维护、首条治理/任务状态链、运行中取消、强杀启动接管与 control 恢复重启，但完整业务闭环条件仍未满足，不得进入壳集成 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-73 | 🟡 | Vitest 14 个文件 189 项；Chromium/Firefox mock smoke 8/8；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium 与 Firefox/真实 `galleryd` 整条运行器各为 14/14、同 AppDirs 恢复重启与 Source guard | 管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全、断线与单帧 sequence gap 恢复、维护、首条治理/任务状态链、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启已用 Chromium/Firefox 纳入 CI workflow；EV-66 另以两浏览器全链锁定扫描/Watcher 与维护终态同步；其余治理、真实移动设备/屏幕阅读器仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-73 又覆盖管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全、断线与单帧 sequence gap 的 snapshot 恢复、维护、规则绑定状态、作品人工解绑/撤销、retry-backoff Job、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启，并让完整链在桌面 Firefox 通过、关闭扫描/Watcher 和维护终态同步竞态。但真实移动设备/屏幕阅读器、窄屏侧栏焦点陷阱、其余治理路径与正式可用性 Gate 均未完成 |
| 测试体系缺口 | EV-54～EV-73 已让管理自举、画廊/媒体、CustomCover、规则 lifecycle/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全资源管理、断线与单帧 sequence gap 恢复、维护、首条治理/任务状态链、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启的 Chromium/Firefox 真实后端 E2E 进入 CI workflow，并保持 mock smoke 与真实证据分层；EV-45 已让阶段 4 testlab Correctness 进入普通 `go test`，但 500k 性能/十来源矩阵仍是人工门禁；全仓库仍无正式 Fuzz，部分平台包仍缺直接测试。完整弱网抖动矩阵、其余治理、真实存储崩溃恢复等路径仍未进入真实后端持续门禁 |
...
| 3 | 阶段 6 | 在 EV-54～EV-73 的 Chromium/Firefox 管理自举、画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全、断线与单帧 sequence gap 恢复、维护、首条治理/任务状态、运行中 Scan/Hash 级联取消、进程强杀启动接管与 control 恢复实际重启门禁上，继续覆盖其余治理、完整弱网抖动矩阵、可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
