...
CanonicalMedia/MediaProjection 的 `ordinal` 只表达作品内非负的内容顺序；封面身份不得借用 `-1` 或其它负数哨兵编码，必须由独立的规则封面与有效封面引用表达。
...
多候选或重复稳定键会持久化一个 Binding issue，携带实体类型、来源稳定键、候选 Canonical 实体、命中信号与置信摘要（不含绝对路径或 metadata 原文）、状态与乐观版本。issue 以候选指纹去重：相同证据不重复产生，被忽略的记录不再打扰；证据变化时旧 issue 标为 superseded 并新建；来源候选在成功扫描后未再出现则标为 stale。默认身份歧义阻塞该 Source publication，用户经 API 修复后重扫方可发布。人工修复以 `control.db` 的 Binding 变更表达：绑定到某候选写入 active Binding，创建新实体或保持分离以 manual_unbound 排除候选，忽略仅转 dismissed；下一次扫描据此重建投影，撤销只回退本次 Binding 变更且不触碰 Canonical 用户事实。人工解绑的撤销请求必须显式携带 Work 或 Media 实体类型，并只恢复该类型下唯一的 `manual_unbound` Binding；若已经存在替代的 active Binding、候选不存在或同一稳定键出现多个可恢复候选，必须以结构化冲突或未找到失败，不得靠相同 `source_key` 猜测跨实体目标。
...
封面解析同时保留两层事实：规则执行得到的 `CoverPath` 必须映射为同一 SourceWork 内稳定的 SourceMedia 引用，并在 Binding 解析后得到同一 CanonicalWork 内的 CanonicalMedia；查询投影再把规则封面与 Overlay 组合为 publication 冻结的有效封面。当前有效值选择顺序为：仍属于该 Work 的 `CustomCover` 优先，否则回退规则封面。失效 `CustomCover` 的用户事实和错误状态继续保存在 `control.db`，不得因回退而删除或改写。
...
- CustomCover 失效：保留用户选择及错误状态，当前查询投影回退到规则封面，不自动删除记录。
