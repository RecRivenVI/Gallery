...
| 性能测试（Reference Performance） | 阶段 3、4 | `internal/query/reference_performance_test.go` 微基准、`tools/testlab` 生产 Store/HTTP 矩阵及 `publication-perf`（均需显式运行） | EV-23/35/36/51/52/96～101/147/150/151 均明确区分方向性测量、执行器预检与正式 Gate | 🟡 | 500k/十来源/双关系 publication 1%/10%/50% 各 20/20；同形状 Query warm/cold-process 各 63/63、0 failure/timeout，报告均有原子身份与 SHA-256 | publication 与 Query 当前执行矩阵已完成；分类/候选量 P95 阈值、结构化过滤计划、维护、完整哈希和 Degradation 结果仍未完成，不能当作完整门禁结论 |
...
| 1 | 阶段 4 收尾 | publication 与同形状 Query warm/cold-process 正式矩阵已完成；冻结查询分类/候选量阈值和结构化计划，执行 Degradation、维护/哈希预算并完成 API 接口冻结 | publication 60/60、Query warm/cold-process 各 63/63，全部失败/超时 0；完整 Reference Gate 仍取决于阈值冻结、计划门禁与尚未执行的 Degradation/维护/哈希 |
