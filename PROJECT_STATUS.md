...
| 阶段 4：查询与媒体 | 🟡（主线代码完成，部分参数未冻结） | 🟡（正确性收口完成，500,000 规模正式压力测试已执行） | 搜索、排序、分页、显式规则/有效封面、媒体读取/下载、缩略图生成全部有代码闭环；EV-51/52 降低搜索与 publication 开销，EV-87/88 用 catalog v20 窄候选把 500k 单 Creator 受限回退降至 0.55～1.00 ms P95 | 正式十来源变化 publication/并发/Degradation 矩阵仍未完成；50,000 Creator 全量列表最坏回退复测 P95 最高 9.31 s，排序权重、Total、租约和正式 API Freeze 尚未冻结 | 完成正式性能矩阵与接口冻结 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| 浏览/搜索/作品/作者/媒体、Overlay、任务、规则、安全与维护界面 | 需要 | 以 OpenAPI/HTTP snapshot 为事实源，同源嵌入 `galleryd`；列表/详情/封面/媒体沿用同一 publication | `web/src/gallery/pages/*`、`web/src/gallery/components/*`、`web/src/manage/pages/*`、`web/src/manage/rules/*`、`web/src/api/*`、`internal/webapp` | 🟠（EV-54～EV-77 的真实后端业务/治理链及 EV-78～EV-85 的窄屏/弱网/设计重构/媒体背压切片已建立） | Vitest、Playwright mock、真实后端 Chrome/Edge Personal/LAN 与 Chromium/Firefox 完整链 | EV-38～EV-40、EV-42、EV-54～EV-85、ADR-009 | 🟡 | 主要合成业务闭环已进入 Chromium/Firefox 真实后端 E2E；真实移动设备、人工屏幕阅读器、跨设备 LAN 与其余弱网矩阵仍未覆盖 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-85 | 🟡 | Vitest 14 个文件 202 项；Chromium/Firefox mock smoke 14/14；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium 与 Firefox/真实 `galleryd` 整条运行器各执行 20 个实际测试、同 AppDirs 多阶段重启与 11 个治理子根 Source guard；EV-79 另有 WSL Linux Chromium 320/360/390/412px 产物探针 | 真实后端持续链覆盖管理自举、画廊/媒体、规则、安全、维护、治理、取消、重启、连续网络切换、一次查询中断/受控延迟、同 origin 服务长停机、双入口重构及媒体背压恢复；mock smoke 另覆盖双入口桌面/390×844 导航、axe、320px 溢出、迟到旧分页隔离与跨旧预算重连。真实移动设备/屏幕阅读器和其余弱网矩阵仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-77 覆盖主要真实后端业务与治理持续链，EV-78～EV-85 又关闭窄屏导航焦点、Linux Chromium 320px Grid 溢出、弱网恢复、双入口重构及真实媒体背压恢复缺口。但真实移动设备/触控、人工屏幕阅读器、全页面可访问性与正式可用性 Gate 均未完成 |
| 测试体系缺口 | EV-54～EV-85 已让主要业务/治理、断线/gap/连续网络切换、一次查询中断、同 origin 服务长停机、媒体背压、取消、强杀接管、恢复重启及双入口重构的 Chromium/Firefox 真实后端 E2E 进入 CI workflow，并保持 mock smoke 与真实证据分层；EV-45 已让阶段 4 testlab Correctness 进入普通 `go test`，但 500k 性能/十来源矩阵仍是人工门禁；全仓库仍无正式 Fuzz，部分平台包仍缺直接测试。带宽、随机延迟/丢包分布、代理/移动网络等其余弱网矩阵与真实存储崩溃恢复仍未进入持续门禁 |
...
| 3 | 阶段 6 | 在 EV-54～EV-85 的 Chromium/Firefox 真实后端业务/治理/网络/媒体背压链和重构后窄屏门禁上，继续覆盖带宽、随机延迟/丢包分布、代理/移动网络等其余弱网矩阵、全页面可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
