...
| 阶段 6：Web/PWA 界面 | 🟠（页面代码基线与双入口设计重构已实现） | 🟡（隔离 Chromium/Firefox 真实后端 E2E 与当前全路由自动 axe 已建立；正式 Gate 未通过） | 同源 Web/PWA 覆盖浏览与管理页面；主要业务/治理链已有双浏览器真实后端持续 E2E；当前 19 条路由在桌面/窄屏共 38 个状态/浏览器通过 axe | 浏览器业务链仍使用合成 Source；真实存储浏览器链、其余弱网矩阵、真实移动/触控、人工屏幕阅读器、缩放/高对比及交互状态组合未完成 | 扩大真实业务与可访问性门禁，不进入桌面壳 |
