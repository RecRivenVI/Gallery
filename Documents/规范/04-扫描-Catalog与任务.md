...
扫描请求快照新增可选 `verificationTargets` 列表，每项冻结 `mediaId`、`sourceId`、规范化 `relativePath` 与请求时刻的 `observationFingerprint`：

```json
{
  "scanProfile": "incremental",
  "verificationTargets": [
    {"mediaId": "...", "sourceId": "...", "relativePath": "...", "observationFingerprint": "..."}
  ]
}
```
...
- 冻结字段不只是持久化，执行阶段必须真正验证：目标的 `observationFingerprint` 与执行时刻的新鲜观察不一致（文件在排队期间被替换/截断/改动，或 Catalog 记录的确认状态已经变化）、目标未出现在本次 discovery 结果中（被移动、改名或删除）、目标相对路径实际解析出的 CanonicalMedia 与冻结的 `mediaId` 不一致，三者任一成立都必须让整个 Job 失败为对应的稳定结构化 code，不得静默确认成一个不是请求方原本指定的对象；多个 target 时任一验证失败都不得让 Job 整体成功。
...
幂等身份至少绑定：CanonicalMedia ID、Source ID、规范化相对路径、size、mtime、当前内容确认状态、请求所依据的 `query_publication_id`。重复请求语义：

| 状态 | 结果 |
| --- | --- |
| 同 observation 的 queued/running Job | 返回同一 Job |
| 同 observation 已完成，且当前媒体已确认 | 稳定 `CONFLICT`（在 `GetMediaAt` 已确认状态时提前返回，不再走到 Job 创建） |
| observation 已变化（size/mtime/确认状态/publication 不同） | 幂等键随之不同，不复用旧 Job |
| 目标 publication 已过期或不存在 | `CURSOR_EXPIRED` |
