...
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携、恢复/回滚及首条真实升级边界） | 🟡（本地制品、恢复、schema 23→24/反向拒绝及轮换失败 smoke 通过，正式 Gate 未通过） | 精确干净提交可生成同源双前端 ZIP、三份 SBOM、清单、摘要与签名状态门禁；正常/损坏备份、真实 schema 23→24/反向拒绝，以及 Windows 首次轮换被拒绝时保留当前库均已通过 | 正式签名、安装/更新、更多支持版本跨度、磁盘满/落位后失败/中断回滚、平台矩阵及桌面壳未完成 | 完成 Windows RC 门禁 |
