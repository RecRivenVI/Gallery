...
每次完成下列任一大型开发后，必须检查并同步更新 `README.md` 与 `PROJECT_STATUS.md`：
...
大型开发完成后的用户文档同步不得采用追加补丁式维护。Agent 必须重新审视并重写 `README.md` 与 `PROJECT_STATUS.md` 中所有受影响的摘要、章节、表格行、状态、限制和后续计划，删除或修正已经失效的表述。不得仅在文档末尾追加日期说明、新 EV 链接或"现已完成"注释来掩盖正文仍然过时的问题。具体禁止：

- 只在 README 或 `PROJECT_STATUS.md` 末尾追加一条日期记录；
- 保留旧状态表，再在下方补一句"现已完成"；
- 用新的注释覆盖仍然存在的过时正文；
- 只增加一个新 EV 链接，而不修正被该 EV 推翻的旧结论；
- 复制新的阶段摘要而让旧摘要并存；
- 让 README 总体表和 `PROJECT_STATUS.md` 总体表出现不同状态。
