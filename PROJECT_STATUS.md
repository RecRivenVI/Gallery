...
| 阶段 5：账户、安全与多客户端 | 🟠（代码与合成安全收尾已实现；恶意输入缺陷已收口） | 🟡（Personal、同机 LAN、真实进程树与显式真实 ffprobe 补证，正式 Gate 未通过） | EV-37/EV-38/EV-44/EV-48 的安全收口之外，EV-60 已把 Session、API Token、Share、allow/deny Grant、账户停用恢复和精确 Session 吊销接入隔离真实浏览器；EV-86 又让 Creator/Library 聚合封面遵守 deny、Token scope 与独立 `media.read`；EV-114 证明输出溢出/超时会终止真实 OS 进程树，EV-115 再覆盖真实 ffprobe 的输出、超时与截断容器 | 真实 LAN 多设备、目标低端设备 Argon2id、恶意媒体语料库、生产 ToolDiscovery/版本允许列表及 OS 级 CPU/内存门禁仍未完成；同机 loopback 不能替代正式 Security Gate | 完成外部设备与恶意资源门禁 |
...
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携、恢复/回滚及首条真实历史升级边界已建立） | 🟡（本地制品、schema 23→24/反向拒绝、恢复失败 smoke、包内双失败及三类进程恢复门禁通过，正式 Gate 未通过） | EV-103～105 完成便携、同源切换、正常/损坏备份；EV-106 证明真实 schema 23→24、旧程序拒绝新 migration 与当前程序复启；EV-107/110 覆盖轮换/落位拒绝；EV-109/119 关闭落位后连续性缺口；EV-121/122 关闭当前库缺失与安全收尾中断窗口；EV-123 再验证结果记录失败和 pending 删除 sharing violation 的进程级 fail-closed/恢复 | Authenticode、安装/更新、CredentialStore、更多支持版本跨度、磁盘满/ACL/低权限、便携同进程双 Rename、实际强杀/断电、平台矩阵及桌面壳均未完成 | 先完成签名、其余失败回滚和 Windows RC 门禁 |
...
| 威胁模型测试（路径攻击、恶意元数据、恶意媒体文件、限流） | 需要 | 认证爆破、Host/Origin/CSRF/Content-Type、WS 防滥用，以及路径/metadata/媒体正文/恢复输入均进入正式合成测试；真实测试二进制覆盖进程树强杀，显式真实 ffprobe 覆盖版本输出基线、输出溢出、超时与截断 MP4 | `internal/auth/security_test.go`、`internal/transport/httpapi/security_api_test.go`、`internal/contract/realtime/envelope_test.go`、`internal/toolrunner/{integration_test.go,actual_ffprobe_test.go}`、`internal/platform/process/controller_test.go`、`tools/testlab/stages/stage5/security` | ✅（合成攻击矩阵 + 真实 OS 进程树/ffprobe 切片） | 合成安全回归 + 高重复/race + Windows/WSL2 真实进程树 + Windows 显式 ffprobe | EV-09、EV-37、EV-114、EV-115 | 🟡 | 单个合成截断容器不是恶意语料库；生产 ToolDiscovery/版本允许列表和 OS 级 CPU/内存硬限额仍缺，也不代表真实 LAN；整体 Security Gate 未通过 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| Windows 正式支持（真实运行、升级、崩溃恢复、签名） | 需要 | 增加无壳便携测试制品、恢复/回滚及首条真实历史升级边界作为 RC 前基线 | `pkg/galleryversion`、Windows 发行脚本、`tools/testlab/cmd/{portable,historical}-upgrade`、Windows CI | 🟠 | 精确干净提交构建、版本/摘要/SBOM/签名事实、同 AppDirs 强杀重启、正常/损坏备份、真实 schema 23→24/反向拒绝、Windows 轮换/落位失败、包内真实 Win32 双失败、当前库缺失 pre-placement fail-closed、安全收尾续接，以及结果记录/pending 删除失败的进程恢复 | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-123 | 🟡（本地制品、恢复、单条历史升级、恢复失败 smoke、包内双失败及三类恢复进程门禁通过） | 仍未完成正式签名、安装/更新、更多支持版本跨度、磁盘满/ACL/低权限、便携同进程双 Rename、实际强杀/断电、多用户及完整 Windows 平台门禁 |
...
| 安装包、签名、SBOM（软件物料清单）、依赖安全检查 | 需要 | 先建立 Windows x64 便携 ZIP、恢复/回滚与真实历史迁移门禁，再完成安装与签名 | EV-103 已生成三个 CycloneDX SBOM、清单、包内/外 SHA-256，构建器支持 Authenticode fail-closed；EV-104～107 覆盖同源切换、正常/损坏备份、真实 schema 23→24/反向拒绝和首次轮换失败；EV-109/110/119/121～123 增加落位后及 pre-placement 连续性 fail-closed、真实候选落位拒绝、包内真实双失败、安全收尾续接与状态文件失败的便携进程恢复；CI 继续执行依赖审计/漏洞扫描 | 🟠（部分） | 本地正向/负向制品 smoke、恢复/回滚、单条真实历史升级边界、真实 Windows 落位失败安全回滚、包内双 sharing-violation、当前库缺失/stale incoming、安全收尾和结果记录/pending 删除失败的进程恢复；Windows CI 与手工来源证明 workflow | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-123、`ci.yml`、`windows-portable.yml` | 🟡 | 当前精确包仍为 `unsigned`；无安装器、证书/时间戳、自动更新、完整支持版本跨度或签名后平台证据，不能作为 RC |
...
|---|---|---|---|---|---|---|
| 全部 Go 测试（783 个顶层 `Test*`/`Benchmark*`/`Example*` 函数，覆盖 66 个目录/包） | 贯穿各阶段 | `cmd`/`internal`/`pkg`/`tools` 共 182 个 `*_test.go` 文件 | `scripts/Check.ps1` 每次运行 | ✅（在 CI 上持续运行并要求全部通过） | Windows + Ubuntu（GitHub Actions） | 绝大多数是模拟/合成数据；部分显式门禁使用真实 OS/工具/Source，局限分别登记；Argon2id benchmark 需手动 `-bench`，CI 从不执行 |
...
| Windows/Linux/macOS/Docker 平台支持 | 阶段 7 | Windows 增加本地便携测试包、恢复/回滚、真实 schema 23→24/反向拒绝、轮换/候选落位失败、包内双失败、当前库缺失/stale incoming、安全收尾及状态文件失败进程门禁；CI 仍只有 Windows + Ubuntu | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-123、ADR-007 四级支持成熟度表 | 🟡（Windows 增加制品/恢复/升级/多条真实失败门禁，Linux 仍停在 CI 层级）/ ⏳（macOS/Docker） | 当前工作站 Windows x64 + GitHub Actions 配置 | 未达到"发行候选"或"正式支持"级别；本地无签名包、未覆盖便携同进程双 Rename 失败与 CI Linux 都不能外推为正式平台支持 |
| 安全（认证、授权、Web 边界、路径穿越、恶意元数据/媒体、限流） | 阶段 5 | 正式生产包覆盖账户/Token/Grant/Session/WS/Web 与合成攻击；Work 查询逐成员授权；真实 Chrome/Edge 已覆盖 Personal/LAN 主路径和吊销；外部工具输出/超时已覆盖真实 OS 进程树及显式真实 ffprobe | EV-09、EV-37、EV-38、EV-44、EV-60、EV-114、EV-115 | 🟡 | Windows 合成与浏览器；Windows 真实进程树/ffprobe 重复；WSL race；隔离 Personal/LAN Chromium 安全管理链 | 真实物理 LAN 多设备、目标低端设备、恶意媒体语料库、生产 ToolDiscovery/版本允许列表与 CPU/内存硬限额未完成，整体 Gate 未通过 |
...
| 发布/签名/SBOM | 阶段 7 | Windows 便携包、SBOM、清单、摘要、可选 Authenticode、恢复/回滚及真实历史升级边界已进入脚本与 CI；EV-109/110/119/121～123 补齐落位后/pre-placement 连续性、真实候选落位安全回滚、包内双失败、安全收尾续接和状态文件失败的便携进程恢复门禁 | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-123、Windows 发行脚本与 workflow | 🟡（未签名测试基线） | 本地精确干净 ZIP 正向/负向、正常/损坏备份、schema 23→24/反向拒绝、轮换/候选落位失败、包内双失败、当前库缺失/stale incoming、安全收尾及结果记录/pending 删除失败的进程 fail-closed/重试；远端 workflow 待当前提交触发 | 正式证书/时间戳、安装器、更新、更多支持版本跨度、磁盘满/ACL、便携同进程双 Rename、实际强杀/断电及 RC 仍不存在 |
...
| 已修正的外部工具测试漂移（EV-114/115） | 已新增 Argon2id benchmark，但只在当前工作站取得 EV-38 数据且 CI 从不执行；全仓库仍无正式 Fuzz。此处旧文称 `internal/toolrunner` 的「有界输出/超时/强杀」测试从未真正触及上限，但提交 `2af146a` 早已让 1,024-byte 输出上限和 3 秒超时驱动真实父子孙进程树终止，EV-114 重新复验；EV-115 又用显式真实 ffprobe 证明版本输出确实超过 128 bytes，并覆盖 128-byte 溢出、3 秒超时与截断容器 | 已关闭“上限未被触及”和“完全没有真实工具补证”的文档缺口；生产 ToolDiscovery/版本允许列表、恶意媒体语料库、CPU/内存硬限额、目标低端设备 Argon2id 与 Fuzz 仍不能算完成 |
...
| 发行缺口 | EV-103 已有未签名 Windows 便携 ZIP、三个 SBOM、摘要与 smoke；EV-104～107 再覆盖正常/损坏备份、真实 schema 23→24/反向拒绝及首次轮换失败；EV-109/110/119 关闭落位后空库风险并证明安全回滚/包内双失败，EV-121/122 关闭当前库缺失与安全收尾中断窗口，EV-123 再覆盖结果记录失败和 pending 删除 sharing violation 的真实进程恢复。仍没有安装包、正式代码签名/时间戳、自动更新、CredentialStore、完整支持版本跨度、磁盘满/ACL/低权限、便携同进程双 Rename、实际强杀/断电和平台支持证据 |
...
| 4 | 阶段 7 | 在 EV-103～107、EV-109/110/119/121～123 的便携、恢复/回滚与真实 schema 23→24 基线上完成 Authenticode、支持版本跨度、磁盘满/ACL、便携同进程双 Rename、实际强杀/断电及 Windows RC；随后推进 Linux 原生、macOS、Docker、SMB/NAS 和安装发行 | 当前关闭制品编排、正常/损坏备份、一条相邻历史边界、轮换/落位失败、包内双失败、当前库缺失 pre-placement fail-open、安全收尾续接及两类状态文件失败进程恢复；签名、其余进程级异常和平台支持仍是正式发行阻断项 |
