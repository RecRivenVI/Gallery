...
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口与首批真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、画廊端与管理端双入口；EV-54～EV-77 覆盖主要业务、治理、取消和重启恢复链，EV-78/EV-79 建立窄屏焦点和 320px Grid 溢出回归，EV-80 修复长离线耗尽预算与 Firefox 丢失 4401 后认证态不收敛，并加入连续三轮 offline/online 门禁 | 门禁仍只用合成 Source；真实存储取消与崩溃恢复响应、真实 `MEDIA_READ_BUSY`、其余完整弱网矩阵、触摸设备与屏幕阅读器未验证；画廊端无 DOM 虚拟化 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| 浏览/搜索/作品/作者/媒体、Overlay、任务、规则、安全与维护界面 | 需要 | 以 OpenAPI/HTTP snapshot 为事实源，同源嵌入 `galleryd`；列表/详情/封面/媒体沿用同一 publication | `web/src/gallery/pages/*`、`web/src/gallery/components/*`、`web/src/manage/pages/*`、`web/src/manage/rules/*`、`web/src/api/*`、`internal/webapp` | 🟠（EV-54～EV-77 的真实后端业务/治理链、EV-78/EV-79 的窄屏门禁及 EV-80 的连续网络恢复已建立） | Vitest、Playwright mock、真实后端 Chrome/Edge Personal/LAN 与 Chromium/Firefox 完整链 | EV-38～EV-40、EV-42、EV-54～EV-80、ADR-009 | 🟡 | 主要合成业务闭环已进入 Chromium/Firefox 真实后端 E2E；真实移动设备、人工屏幕阅读器、跨设备 LAN 与其余弱网矩阵仍未覆盖 |
| 实时通道（WebSocket → HTTP snapshot 恢复） | 需要 | WS 只作提示，断线/gap 以 HTTP snapshot 恢复 | `web/src/shared/realtime.tsx`、`internal/contract/realtime` | ✅（EV-40 修复 `WS-1`/`WS-2`，EV-71 补齐单帧 gap，EV-80 修复长离线和安全 close code 丢失） | 真实 Chrome/Edge 连接/事件/CSP；隔离 Chromium/Firefox 真实断线、单帧 gap、连续三轮 offline/online 与 bootstrap/snapshot 收敛 | EV-39、EV-40、EV-60、EV-71、EV-80 | 🟡 | 已覆盖一次断线、单帧 gap、长离线预算暂停、连续三轮网络切换和 Firefox 1006 吊销收敛；带宽/延迟/丢包/服务长停机/移动网络等其余完整弱网矩阵仍未覆盖 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-80 | 🟡 | Vitest 14 个文件 196 项；Chromium/Firefox mock smoke 10/10；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium 与 Firefox/真实 `galleryd` 整条运行器各执行 17 个实际测试、同 AppDirs 多阶段重启与 11 个治理子根 Source guard；EV-79 另有 WSL Linux Chromium 320/360/390/412px 产物探针 | 真实后端持续链覆盖管理自举、画廊/媒体、规则、安全、维护、治理、取消、重启及连续网络抖动恢复；mock smoke 另覆盖双入口桌面/390×844 导航、axe 与 320px 溢出。真实移动设备/屏幕阅读器和其余弱网矩阵仍未覆盖 |
...
| WebSocket 实时通道 | `/ws/v1` 推送任务、publication、安全吊销等事件 | EV-40 修复握手与信封字段；EV-54 以真实 Job 锁定事件后的 HTTP snapshot；EV-60 覆盖一次断线，EV-71 覆盖单帧 gap，EV-80 再覆盖长离线暂停预算、Firefox 1006 吊销收敛和连续三轮 offline/online。其他事件类型及带宽/延迟/丢包/服务长停机等完整弱网矩阵仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-77 覆盖主要真实后端业务与治理持续链，EV-78/EV-79 关闭窄屏导航焦点和 Linux Chromium 320px Grid 溢出，EV-80 关闭长离线预算与 Firefox 1006 吊销收敛缺陷。但真实移动设备/触控、人工屏幕阅读器、全页面可访问性与正式可用性 Gate 均未完成 |
| 测试体系缺口 | EV-54～EV-80 已让主要业务/治理、断线/gap/连续网络切换、取消、强杀接管与恢复重启的 Chromium/Firefox 真实后端 E2E 进入 CI workflow，并保持 mock smoke 与真实证据分层；EV-45 已让阶段 4 testlab Correctness 进入普通 `go test`，但 500k 性能/十来源矩阵仍是人工门禁；全仓库仍无正式 Fuzz，部分平台包仍缺直接测试。带宽/延迟/丢包/服务长停机等其余完整弱网矩阵与真实存储崩溃恢复仍未进入持续门禁 |
...
| 3 | 阶段 6 | 在 EV-54～EV-80 的 Chromium/Firefox 真实后端业务/治理/连续网络恢复链和窄屏门禁上，继续覆盖带宽/延迟/丢包/服务长停机等其余弱网矩阵、全页面可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
