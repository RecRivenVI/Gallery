...
| 阶段 4：查询与媒体 | 🟡（主线完成，部分参数未冻结） | 🟡（正确性收口完成，500,000 规模正式压力测试已执行） | 搜索、排序、分页、显式规则/有效封面、媒体读取、缩略图及用户/治理 Creator keyset 均有代码闭环；500,000 规模 Correctness/Cursor 通过 | 正式十来源变化 publication/并发/Degradation 尚未完成；排序权重、Total、租约和兼容版本策略仍待 API Freeze | 性能矩阵与接口冻结 |
