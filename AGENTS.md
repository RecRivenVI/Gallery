...
- 当前仓库已有正式产品代码（`cmd/`、`internal/`、`pkg/`、`web/`）。阶段 0 契约骨架、Walking Skeleton、Architecture Proof 正确性切片、阶段 1「领域和数据所有权」、阶段 2「规则闭环」、阶段 3「扫描、任务和 Catalog」与阶段 4「查询和媒体」均已完成代码与合成 Correctness 实现；阶段 5「账户、安全和多客户端」已完成代码/合成安全收尾，并取得 Chrome/Edge 同机 Personal/LAN 主路径与当前工作站 Argon2id 补证；EV-48 又关闭规则递归深度崩溃及空格式、Windows 路径、Range、Cursor 的非规范输入缺口，EV-60 再以隔离 Chromium/真实 `galleryd` 覆盖 Personal Session/API Token/Share 和独立 loopback LAN 账户/Grant/Session 管理链，但正式 Security Gate 尚未通过。阶段 6 已实现 React/TypeScript Web/PWA 页面代码基线、OpenAPI 生成客户端、同源嵌入资产、静态壳 PWA，以及认证、浏览/媒体、Overlay、任务、规则、安全和维护页面；EV-54～EV-58 已把管理自举、publication-bound 画廊/媒体、CustomCover、规则发布首链、无损文本与 Schema 表单接入隔离 Chromium/真实 `galleryd` 持续门禁，EV-59 又增加 rollback、Package/Version/ParameterSet deprecate、共享参数 CAS/Impact 与 Binding/Job 执行快照不变量，EV-60 补齐首条安全管理写链与真实 WebSocket 断线后的 HTTP snapshot 恢复，EV-61 覆盖 control 备份、恢复干跑/待重启登记和 Catalog GC dry-run，EV-62 再覆盖 SourceRuleBinding 暂停/恢复、作品人工解绑/撤销及 retry-backoff Job 取消/同 ID 重试与 Attempt 历史，EV-64 进一步以同一隔离 AppDirs 的实际重启证明 control 恢复生效和 `restore.finalize` 审计，EV-65 把完整隔离链扩展到桌面 Firefox 并纳入 CI，EV-66 又关闭扫描/Watcher 与维护终态同步竞态，EV-67～EV-69 再补齐全部现有 primitive config 的 Schema 字段、当前草稿 Dry Run/Explain/Trace 及按字段撤销链，但正式 Web Gate 尚未通过。阶段 4 的 EV-35/EV-36 Correctness/Cursor 结论保持；EV-51 已把 500,000 合成 WorkProjection 的单并发搜索候选路径降到两位数毫秒 P95，EV-52 又把完整候选验证与 publication 指针切换拆分并以持久验证封印 fail-closed，但尚未覆盖正式十来源完整关系、1%/10%/50% 变化 publication 多样本、并发与 Degradation 矩阵，因此 Reference Performance Gate 仍未通过，Gank/Pawchive 有界验证也仍未完成。
- **[EV-39](Documents/证据/验证记录.md)（2026-07-23）的真实浏览器复核下调了阶段 6 的完成度陈述**：`/ws/v1` 在 Chrome/Edge 中 100% 握手失败（服务端对 WebSocket 强制要求浏览器不会发送的 `Sec-Fetch-Site` 头），前端信封字段名与 `internal/contract/realtime` 不符，且 6 个前端 capability 名不在后端权威词表中，导致 Overlay 编辑、任务取消/重试、Library 创建、Source 登记、按需内容确认与全部治理动作对任何主体都不渲染。[EV-40](Documents/证据/验证记录.md) 已修复这些阻断项并经真实 Chrome/Edge 复验；EV-54～EV-69 已建立管理自举、publication-bound 画廊/媒体、CustomCover、规则发布/回滚/弃用、ParameterSet、无损文本、Schema 表单、当前草稿 Dry Run/Explain/Trace 与按字段撤销、安全资源管理、断线恢复、维护、规则绑定状态、作品人工解绑/撤销、retry-backoff Job 取消/重试及 control 恢复实际重启的 Chromium/Firefox 真实后端 E2E，并关闭显式扫描与 Watcher 状态脱节、维护 Job 终态不刷新的持续门禁竞态，但**仍不得把阶段 6 描述为「已完成业务闭环」**——参数 Schema/tests/extensions 等任意 JSON 子树的完整可视化构建、其余治理决策、长任务运行中取消与进程中断恢复、真实设备与可访问性门禁仍未完成。[EV-44](Documents/证据/验证记录.md) 又关闭 `AUTHZ-1` 与 `QRY-1`，[EV-45](Documents/证据/验证记录.md) 关闭 `TEST-2`，[EV-46](Documents/证据/验证记录.md) 关闭 `MED-1` 与 `SEC-3`。**EV-39 登记的缺陷至此全部关闭**；EV-46 另行发现并修复了三项新缺陷（`LINK-1` Windows 目录联接被识别为普通文件、`TX-1` WAL 下读后写事务的过期读快照失败、迁移预算门禁用固定墙钟导致不可复现），详见 EV-39、EV-40、EV-44、EV-45 与 EV-46。
...
- [EV-68](Documents/证据/验证记录.md)（2026-07-28）把既有 `rules.debug` 后端契约接入规则草稿页：当前未保存规范 JSON、无损参数与显式合成 Sample 可分别执行 Dry Run、Explain、Trace，精确数字在请求和响应中不经 JavaScript `Number`，输入变化后旧结果失效隐藏；Chromium/Firefox 隔离真实后端链已覆盖该闭环。该切片不读取真实 Source/metadata，也不完成参数 Schema/tests/extensions、完整治理或 Web Gate。
- [EV-69](Documents/证据/验证记录.md)（2026-07-28）为 Schema 表单建立基于本地 `baseRevision` 精确快照的按字段撤销：普通字段按 JSON Pointer 列出差异，数组长度变化与 opaque 根字段作为原子字段，非法无损 JSON 也可独立恢复；远端 revision 漂移时不会把后来内容冒充撤销基线，全部字段恢复后精确回到原草稿字节。组件及 Chromium/Firefox 隔离真实后端链已覆盖该闭环；它不提供远端合并，也不完成参数 Schema/tests/extensions 的任意可视化构建或 Web Gate。
- [EV-50](Documents/证据/验证记录.md)（2026-07-27）修复同一 Creator 横跨多个 Source 时全局 Creator 封面被错误扩散到各 Source 的资源边界缺陷，并让 Creator/Source 复用一次物化候选连接；Source 现在严格只引用自身 Work/Media。该轮只有合成行为与查询计划证据，没有 500,000 正式性能结果；Creator/Library 聚合封面按主体 deny/Token scope 与独立 `media.read` 裁剪仍是未完成授权项。
...
- 尚未完成：阶段 5 真实物理 LAN 多设备、目标低端设备 Argon2id 和真实恶意资源门禁；阶段 6 真实移动设备/屏幕阅读器、任意规则的完整可视化构建、Binding issue/结构决策/孤儿/media unbind 等其余治理写路径、长任务运行中取消与进程中断恢复、真实 `MEDIA_READ_BUSY`、显式 WS sequence gap/弱网抖动与完整可访问性 E2E；以及真实全量 HDD、SMB/NAS、真实 FileID/`dev+inode`、正式 Reference/Degradation Performance Gate、ranking/total/租约等 PRE_FREEZE 数值、AND/OR canonical 化和平台发行。
...
EV-51/EV-52 在 compatibility baseline 内演进到 catalog v19：v15 增加 Work 标量展示事实，v16 增加 Creator/Source/Library 聚合封面，v17 登记自然排序编码 v2，v18 增加与 FTS5 docid 一一对应的 `work_search_candidates` 窄候选投影，v19 增加候选验证封印与 Overlay candidate 持久创建基线。任何写入/克隆/重投影/Creator merge/自然排序回填和 GC 路径都必须同时维护 WorkProjection、候选与 FTS；完整 Validate 须复核数量、rowid、业务键、候选事实、FTS 文本、成员和封面并在同一 IMMEDIATE 事务内完成聚合封面后写入封印。封印是候选完成标记：所有 Store 内容写入必须在同一事务确认匹配 Job/Source/watermark（Overlay 另以 revision 行中的 `base_overlay_revision_id` 核对创建基线）、目标仍为 staging 且没有封印；Apply/Validate 不得把后续 active publication 漂移误判为候选损坏，同一 Store 的候选生命周期由互斥边界禁止 Stage/Validate/Publish 交错，验证后与发布后的旧 Candidate 一律拒绝原地改写。GC 必须保护仍被 staging Overlay candidate 持久引用的 base publication/revision，直到候选进入 terminal 状态才可回收。Publish 只允许确认匹配封印、候选状态与 active CAS 后切换 publication；调用方替换 persisted base 必须 fail-closed，合法旧基线候选则在 Publish 得到可重试 `CONFLICT`。绕过 Store/AppDirs 进程锁直接写 catalog.db 不属于支持的产品写入边界；显式重验会先核对候选身份再撤销旧封印，失败后不得恢复。该迁移与当前性能结果不代表物理 Schema、API 或数值 Freeze。
...
- 本机交互式验证默认把系统资源占用控制在整机约 50% 以内：重型门禁串行执行，Go/WSL 的 `GOMAXPROCS` 与 `-p`、Vitest `maxWorkers` 不得超过逻辑处理器数的一半，Playwright 保持单 worker，并避免同时运行构建、race、全量扫描等高内存或高 I/O 任务；只有用户明确授权时才提高预算。当前 28 逻辑处理器工作站的本地测试并发上限为 14。
...
阶段 0～4 已完成代码与合成 Correctness；EV-42 又补齐显式规则/有效封面、Work 快照契约和 Web 同快照封面/编辑的合成 Correctness，EV-44 关闭 Work 聚合查询逐成员授权缺口并增加 catalog v12 revision 成员事实，EV-45 再把 1,000 Work 的查询/Cursor/媒体/DerivedAsset Correctness 接入普通 `go test` 持续门禁，EV-51 又用 catalog v18 窄候选修复 FTS 命中后的宽投影查找并取得 500,000 单并发候选基准，但均不改变 API Freeze/Reference Performance 整体结论。阶段 5 安全代码基线及 EV-37 合成证据完整，EV-38 又补充 Chrome/Edge 同机 Personal/LAN 与当前工作站 Argon2id 证据，EV-44 补齐 deny/Token scope/hidden 写权限的合成授权证据，EV-60 再补齐 Personal 与独立 loopback LAN 安全资源管理浏览器链，但正式 Security Gate 未通过。阶段 6 React/TypeScript Web/PWA、同源嵌入资产、静态壳 PWA 和主要页面骨架已实现，EV-54～EV-69 已把管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、Dry Run/Explain/Trace 与按字段撤销、安全管理、真实 WS 断线 snapshot 恢复、维护、规则绑定状态、作品人工解绑/撤销、retry-backoff Job 与 control 恢复实际重启接入隔离 Chromium/Firefox 真实后端持续门禁，并关闭扫描/Watcher 与维护终态同步竞态，但正式 Web Gate 未通过，且 EV-39 证明其「业务闭环」此前被高估；参数 Schema/tests/extensions 等任意 JSON 子树的完整可视化构建、其余治理、长任务取消/中断恢复、真实设备与完整可访问性仍缺。阶段 4 的 EV-35/EV-36 Correctness/Cursor 结论保持：Reference Performance Gate 未通过，Gank/Pawchive 真实有界验证未完成。
...
EV-54～EV-69 已让管理自举、publication-bound 画廊/媒体、CustomCover 真实写后 publication、规则生命周期/ParameterSet、模板驱动 Schema 表单/无损文本、当前草稿 Dry Run/Explain/Trace 与按字段撤销、安全资源管理、真实 WS 断线 snapshot 恢复、维护、规则绑定状态、作品人工解绑/撤销、retry-backoff Job 与 control 恢复实际重启 E2E 以 Chromium/Firefox 进入 CI，并关闭显式扫描/Watcher 状态脱节、扫描期间事件丢失与维护终态不刷新的竞态；当前最大的 Web 门禁缺口转为参数 Schema/tests/extensions 等任意 JSON 子树的完整可视化构建、其余治理、长任务取消/中断恢复、显式 WS sequence gap/弱网抖动、真实设备与完整可访问性。`gallery-rules.json` 所需的三级聚合封面、独立只读文件根、平台呈现/排序集下发均已实现；EV-50 已锁定 Source-local 聚合资源边界并消除 Creator→Source 的二次候选扇出，EV-51 已用窄候选消除 FTS 后逐命中宽投影查找，排序协议 v2 已贯通 title/name、publishedAt、Progress、签名 keyset 与 Web 规则菜单，但尚未取得完整 Reference Performance/API Freeze 证据，Creator/Library 聚合封面的逐主体授权裁剪也未完成。

其后并行关闭阶段 4 性能/API Freeze、阶段 5 真实物理 LAN/目标低端设备/恶意资源门禁和阶段 6 真实移动设备/屏幕阅读器/完整业务 E2E；在这些完成前不要进入桌面壳或发行。真实 HDD/SMB/NAS、FileID、ranking/total/租约等 PRE_FREEZE 数值、AND/OR canonical 化、Wails/Tauri 与跨平台发行仍属后续门禁。
