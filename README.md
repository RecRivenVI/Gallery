...
| 阶段 4：查询与媒体 | 🟡（主线完成，部分参数未冻结） | 🟡（正确性收口完成，500,000 publication 正式矩阵执行中） | 搜索、排序、分页、显式规则/有效封面、媒体读取、缩略图及用户/治理 Creator keyset 均有代码闭环；500,000 Correctness/Cursor 通过，十来源双关系 publication 矩阵已完成 52/60、失败 0，50% 档为 12/20 并持续运行 | publication 的 50% 变化档及 Query 并发/冷缓存、Degradation 尚未完成；排序权重、Total、租约和兼容版本策略仍待 API Freeze | 完成正式 500k 性能矩阵并冻结接口 |
...
| 阶段 6：Web/PWA 界面 | 🟠（页面代码基线与双入口设计重构已实现） | 🟡（隔离 Chromium/Firefox 真实后端 E2E、当前全路由 axe 与模拟强制颜色/400% 等效重排已建立；正式 Gate 未通过） | 同源 Web/PWA 覆盖浏览与管理页面；当前 19 条路由及 5 个关键交互状态在桌面、窄屏或 320px 高对比/文本间距组合通过 axe；EV-129/132 已清除全部依赖审计例外，EV-136～EV-139 已窗口化 Job、Binding issue、orphan candidate、作品、Creator 与文件目录，EV-140 已补齐 HTTP 会话失效回退 | 浏览器业务链仍使用合成 Source；真实存储浏览器链、其余弱网矩阵、真实移动/触控、人工屏幕阅读器、真实浏览器缩放、物理操作系统高对比及结构决策/安全资源等其余管理列表与交互状态组合未完成 | 扩大真实业务与可访问性门禁，不进入桌面壳 |
