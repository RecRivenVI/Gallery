...
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携测试基线、恢复/回滚及首条真实升级边界） | 🟡（本地制品、恢复与 schema 23→24/反向拒绝 smoke 通过，正式 Gate 未通过） | 精确干净提交可生成同源双前端 ZIP、三份 SBOM、清单、摘要与签名状态门禁；同源切换、正常恢复、损坏备份保留当前库，以及真实祖先 schema 23→当前 24 的迁移与旧程序拒绝新库均已通过 | 正式签名、安装/更新、更多支持版本跨度、磁盘满/权限/中断回滚、平台矩阵及桌面壳未完成 | 完成 Windows RC 门禁 |
