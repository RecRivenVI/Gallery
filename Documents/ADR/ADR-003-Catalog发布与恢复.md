...
2026-07-26 将同一决策补齐到 Work→封面媒体引用：规则 `CoverPath` 以 SourceMedia 稳定键进入 Catalog revision，经 Binding 解析的规则 CanonicalMedia 与叠加 CustomCover 后的有效 CanonicalMedia 以显式列进入匹配的 Overlay projection revision；媒体 `ordinal` 只保留非负内容顺序，不再编码封面。`GET /api/v1/works/{workId}` 与 `PublishedWork.coverMediaId` 使用和媒体读取相同的 current/snapshot/租约语义，保证列表、详情、封面正文和媒体列表不能跨 publication 混代。
