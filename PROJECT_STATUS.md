...
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口与首批真实业务链路已实现） | 🟡（隔离 Chromium/真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、画廊端与管理端双入口；EV-54～EV-59 覆盖管理自举、同快照画廊/媒体、CustomCover、规则生命周期/ParameterSet 与 RJSF/AJV 表单，EV-60 覆盖安全管理和真实 WS 断线 snapshot 恢复，EV-61 覆盖备份、恢复验证/登记与 GC dry-run，EV-62 覆盖规则绑定状态、作品人工解绑/撤销和 retry-backoff Job 取消/同 ID 重试，EV-64 以同一隔离 AppDirs 实际重启证明 control 恢复生效 | 门禁仍只用合成 Source；任意规则的完整可视化构建、其余治理决策、长任务运行中取消与进程中断恢复、真实 `MEDIA_READ_BUSY`、显式 WS sequence gap/弱网抖动、Firefox、触摸设备与屏幕阅读器未验证；画廊端无 DOM 虚拟化 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
| 阶段 7：平台适配与正式发行 | ⏳（仅早期实验代码涉及，不属于正式产品） | ⛔ | 无 | Windows 之外的平台、安装包、签名、升级等均尚未开始 | 最后阶段 |

**概览**：Gallery 已有正式后端和同源内嵌 Web/PWA 代码基线，Chrome/Edge 已验证主要认证与浏览器恢复路径；但真实大规模、真实多平台、真实网络硬盘、完整 Security/Web Gate 和正式发行仍未完成。2026-07-23 的独立审计（[验证记录 EV-39](Documents/证据/验证记录.md)）用真实浏览器探针发现阶段 6 的「业务闭环」此前被高估：实时 WebSocket 通道在真实浏览器中 100% 握手失败，网页端多数写入口因 capability 名不符而不渲染。这些阻断性缺陷已在同日的 [EV-40](Documents/证据/验证记录.md) 修复并经真实 Chrome/Edge 复验；[EV-54～EV-59](Documents/证据/验证记录.md) 随后把管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet、无损文本与 Schema 表单接入隔离真实后端持续门禁，[EV-60](Documents/证据/验证记录.md) 再加入安全资源管理、独立 loopback LAN 账户/Grant/Session 和真实 WebSocket 断线 snapshot 恢复，[EV-61](Documents/证据/验证记录.md) 覆盖 control 备份、恢复验证/登记与 Catalog GC dry-run，[EV-62](Documents/证据/验证记录.md) 再覆盖规则绑定状态、作品人工解绑/撤销与 retry-backoff Job 取消/同 ID 重试，[EV-64](Documents/证据/验证记录.md) 又以同一隔离 AppDirs 的实际重启证明 control 恢复生效。这些链路仍使用合成 Source；任意规则的完整可视化构建、其余治理决策、长任务运行中取消与进程中断恢复和跨浏览器/可访问性仍未完成，阶段 6 Web Gate 依旧未通过。
...
| 阶段 6 业务闭环 | 「普通浏览器完成所有业务闭环后，才开始壳集成」 | **完成度被高估后已下调，现恢复首批持续切片** | EV-39 实测发现 WebSocket 与 capability 阻断；EV-40 修复，EV-54～EV-64 又以隔离真实后端覆盖管理自举、扫描 publication、WS→HTTP snapshot、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、维护、规则绑定状态、作品人工解绑/撤销、retry-backoff Job 与 control 恢复实际重启 | EV-39/40 真实 Chrome/Edge 探针；EV-54～EV-64 Chromium/真实 `galleryd` 合成 Source 持续 E2E | 已覆盖规则、安全、维护、首条治理/任务状态链与 control 恢复重启，但任意规则的完整可视化构建及完整业务闭环条件仍未满足，不得进入壳集成 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| 浏览/搜索/作品/作者/媒体、Overlay、任务、规则、安全与维护界面 | 需要 | 以 OpenAPI/HTTP snapshot 为事实源，同源嵌入 `galleryd`；列表/详情/封面/媒体沿用同一 publication | `web/src/pages/*`、`web/src/components/work-cover.tsx`、`web/src/manage/rules/*`、`web/src/api/*`、`internal/webapp` | 🟠（页面骨架已实现；EV-40 已关闭 capability 写入口阻断，EV-42 新增实际封面与 CustomCover 编辑，EV-55～EV-59 补齐真实媒体/CustomCover/规则生命周期/ParameterSet/Schema 表单，EV-60 补齐安全管理首链，EV-61 补齐维护首链，EV-62 补齐规则绑定状态和首条治理/任务状态链，EV-64 补齐 control 恢复实际重启） | Vitest、Playwright mock、真实后端 Chrome/Edge Personal/LAN | EV-38～EV-40、EV-42、EV-54～EV-64、ADR-009 | 🟡 | 合成画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、维护、作品人工解绑/撤销、retry-backoff Job 和 control 恢复实际重启已进入真实后端 E2E；任意规则完整可视化、其余治理、长任务取消/中断恢复、Firefox、真实移动设备与跨设备 LAN 仍未覆盖 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-64 | 🟡 | Vitest 14 个文件 180 项；Chromium mock smoke 4/4；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium/真实 `galleryd` 为 Personal 11/11、LAN 1/1、同 AppDirs 恢复重启与 Source guard | 管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、断线恢复、维护、首条治理/任务状态链与 control 恢复实际重启已纳入 CI workflow；任意规则完整可视化、其余治理、长任务取消/中断恢复、Firefox/真实移动设备/屏幕阅读器仍未覆盖 |
...
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-64 又覆盖管理自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、断线 snapshot 恢复、维护、规则绑定状态、作品人工解绑/撤销、retry-backoff Job 与 control 恢复实际重启。但 Firefox、真实移动设备/屏幕阅读器、窄屏侧栏焦点陷阱、任意规则的完整可视化构建、其余治理路径、长任务取消/中断恢复与正式可用性 Gate 均未完成 |
| 测试体系缺口 | EV-54～EV-64 已让管理自举、画廊/媒体、CustomCover、规则 lifecycle/ParameterSet/Schema 表单、安全资源管理、断线恢复、维护、首条治理/任务状态链与 control 恢复实际重启的 Chromium/真实后端 E2E 进入 CI workflow，并保持 mock smoke 与真实证据分层；EV-45 已让阶段 4 testlab Correctness 进入普通 `go test`，但 500k 性能/十来源矩阵仍是人工门禁；全仓库仍无正式 Fuzz，部分平台包仍缺直接测试。显式 WS sequence gap/弱网抖动、任意规则完整可视化、其余治理、长任务取消与进程中断恢复等路径仍未进入真实后端持续门禁 |
...
| 3 | 阶段 6 | 在 EV-54～EV-64 的管理自举、画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、安全、断线恢复、维护、首条治理/任务状态与 control 恢复实际重启门禁上，继续覆盖任意规则的完整可视化构建、其余治理、长任务运行中取消与进程中断恢复、显式 WS sequence gap/弱网抖动、浏览器/可访问性与真实设备 | 继续扩大真实后端 E2E，再对业务闭环和发布可用性作结论 |
