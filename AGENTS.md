...
EV-51 在 compatibility baseline 内演进到 catalog v18：v15 增加 Work 标量展示事实，v16 增加 Creator/Source/Library 聚合封面，v17 登记自然排序编码 v2，v18 增加与 FTS5 docid 一一对应的 `work_search_candidates` 窄候选投影。任何写入/克隆/重投影/Creator merge/自然排序回填和 GC 路径都必须同时维护 WorkProjection、候选与 FTS；Validate 与 Publish 事务内均须复核数量、rowid、业务键、候选事实和 FTS 文本。该迁移与当前性能结果不代表物理 Schema、API 或数值 Freeze。
