...
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口与首批真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、画廊端与管理端双入口；EV-54～EV-77 覆盖主要业务、治理、取消和重启恢复链，EV-78～EV-81 建立窄屏焦点/溢出、长离线/吊销收敛及迟到 HTTP 查询隔离回归 | 门禁仍只用合成 Source；真实存储取消与崩溃恢复响应、真实 `MEDIA_READ_BUSY`、其余完整弱网矩阵、触摸设备与屏幕阅读器未验证；画廊端无 DOM 虚拟化 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| 浏览/搜索/作品/作者/媒体、Overlay、任务、规则、安全与维护界面 | 需要 | 以 OpenAPI/HTTP snapshot 为事实源，同源嵌入 `galleryd`；列表/详情/封面/媒体沿用同一 publication | `web/src/gallery/pages/*`、`web/src/gallery/components/*`、`web/src/manage/pages/*`、`web/src/manage/rules/*`、`web/src/api/*`、`internal/webapp` | 🟠（EV-54～EV-77 的真实后端业务/治理链及 EV-78～EV-81 的窄屏/弱网切片已建立） | Vitest、Playwright mock、真实后端 Chrome/Edge Personal/LAN 与 Chromium/Firefox 完整链 | EV-38～EV-40、EV-42、EV-54～EV-81、ADR-009 | 🟡 | 主要合成业务闭环已进入 Chromium/Firefox 真实后端 E2E；真实移动设备、人工屏幕阅读器、跨设备 LAN 与其余弱网矩阵仍未覆盖 |
| 实时通道与 HTTP 查询恢复 | 需要 | WS 只作提示，断线/gap 以 HTTP snapshot 恢复；旧 HTTP 查询不得覆盖新路由/搜索/分页 | `web/src/shared/realtime.tsx`、`web/src/gallery/queries.ts`、`internal/contract/realtime` | ✅（EV-40 修复 `WS-1`/`WS-2`，EV-71 补齐单帧 gap，EV-80 修复长离线/close code 丢失，EV-81 隔离迟到查询） | 真实 Chrome/Edge 连接/事件/CSP；隔离 Chromium/Firefox 真实断线/gap/offline；组件与双浏览器生产资产可控迟到响应 | EV-39、EV-40、EV-60、EV-71、EV-80、EV-81 | 🟡 | 已覆盖一次断线、单帧 gap、长离线预算暂停、连续网络切换、Firefox 1006 吊销及成功响应迟到；错误响应乱序、带宽/随机延迟/丢包/服务长停机/移动网络仍未覆盖 |
...
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-81 | 🟡 | Vitest 14 个文件 199 项；Chromium/Firefox mock smoke 12/12；Chrome/Edge 既有真实认证/实时证据；隔离 Chromium 与 Firefox/真实 `galleryd` 整条运行器各执行 17 个实际测试、同 AppDirs 多阶段重启与 11 个治理子根 Source guard；EV-79 另有 WSL Linux Chromium 320/360/390/412px 产物探针 | 真实后端持续链覆盖管理自举、画廊/媒体、规则、安全、维护、治理、取消、重启及连续网络抖动恢复；mock smoke 另覆盖双入口桌面/390×844 导航、axe、320px 溢出与迟到旧分页隔离。真实移动设备/屏幕阅读器和其余弱网矩阵仍未覆盖 |
...
| WebSocket 与 HTTP 恢复 | `/ws/v1` 推送任务、publication、安全吊销等事件；HTTP snapshot/查询仍是事实源 | EV-40 修复握手与信封字段；EV-54 锁定事件后的 HTTP snapshot；EV-60/EV-71/EV-80 覆盖断线、gap、长离线和吊销收敛；EV-81 隔离旧游标通知及迟到分页/详情响应。错误响应乱序、带宽/随机延迟/丢包/服务长停机等完整弱网矩阵仍未覆盖 |
