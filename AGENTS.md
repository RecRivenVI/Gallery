...
出现下列任一情况都必须进行拆分评估，不得因为"属于同一个阶段"就保留为单个提交：
...
以下情况必须积极评估合并，不得为了"每个细小目的独立成提交"而长期保留没有回退价值的中间状态：
...
合并文档提交时允许上升到更高层次的 `scope`（例如 `docs(审计)`、`docs(指南)`、`docs(仓库)`），前提是合并后的标题和正文准确描述完整目的，而不是把多个无关目的并列。
...
后期独立审计发现的缺陷修复通常保留独立提交，因为它们有独立复现、独立验证记录和独立安全或正确性价值；不得为了让历史"看起来从未犯错"而把后期修复折叠回原始阶段提交。
...
`AGENTS.md` 只有在修改独立、面向未来的 Agent 行为规范时才应单独提交；当它只是随同一次事实更新同步当前状态、阶段结论或优先级时，应并入该次事实更新提交。该放宽只作用于文档之间的合并，不改变"Agent 规则修订与生产代码修改必须拆分"这一条：`AGENTS.md` 仍不得与生产代码放进同一个提交。
...
- 标题和正文之间恰好一个空行；
- 阶段任务提交的正文标题只能是一行 `阶段 N 变动内容：`，非阶段提交的正文标题只能是一行 `变动内容：`；
- 已废止的 `开发阶段：阶段 N` 独立字段行不再允许出现，也不得与 `变动内容：` 并列书写；
- 阶段编号必须使用阿拉伯数字，`阶段`、编号与 `变动内容：` 之间各恰好一个半角空格；
- 正文标题必须顶格书写并使用中文全角冒号；
- 正文标题后至少一个列表项；
- 列表使用半角 `- `，每项顶格且独占一个物理行；
- 各列表项之间不得插入空行；
...
- Merge 和 Revert 必须严格使用下文规定的完整专用格式，Revert 额外包含 `撤销对象：` 字段；Revert 的正文标题是否使用 `阶段 N 变动内容：` 取决于被撤销对象是否属于明确编号开发阶段，规则见"开发阶段"与"Merge、Revert 与不兼容变更"两节。
...
阶段任务的阶段编号原则上只写在正文标题 `阶段 N 变动内容：`。只有提交本身专门调整阶段划分、阶段状态或阶段边界时，标题才允许出现阶段编号。
...
凡提交明确属于 Gallery 正式开发阶段，正文标题必须写成：

```text
阶段 N 变动内容：
```

不属于任何编号开发阶段的提交，正文标题只能写成：

```text
变动内容：
```

两者互斥，一个提交只能有一行正文标题。旧格式中独立成行的 `开发阶段：阶段 N` 字段已被废止，任何新提交和任何历史重写结果都不得再出现该行。
...
Merge Commit 描述的是分支历史合并行为，而不是某个开发阶段内的实现切片：其主要语义由合并主题和引入的逻辑变动表达，不是该提交发生在哪个开发阶段。因此 Merge 通常使用非阶段正文标题 `变动内容：`，除非未来规范明确增加特殊 Merge 阶段管理需求。

Revert Commit 使用哪一种正文标题，取决于被撤销的原提交，而不是仓库当前整体所处阶段：若被撤销对象属于明确编号开发阶段（阶段 0～阶段 7），Revert 正文在 `撤销对象：` 之后使用 `阶段 N 变动内容：`；若被撤销对象不属于任何编号开发阶段，则使用 `变动内容：`。普通阶段任务提交必须使用阶段正文标题，Merge 不属于普通阶段任务提交因此通常不使用，Revert 单独按被撤销对象判断，三者不得混为一谈。Merge 与 Revert 的完整专用格式、条件判定和示例见"Merge、Revert 与不兼容变更"一节。
...
- 因此 Merge Commit 必须保留 Merge 结构，通常使用非阶段正文标题 `变动内容：`，除非未来规范明确增加特殊 Merge 阶段管理需求。
...
- 如果被撤销对象属于明确编号开发阶段（阶段 0～阶段 7），必须在 `撤销对象：` 之后使用 `阶段 N 变动内容：`：
...
范围 Revert 的正文标题判定与单一对象一致：仅当范围内全部原提交都属于同一个编号开发阶段时，才在 `撤销对象：` 之后使用该 `阶段 N 变动内容：`；只要范围内存在不属于编号阶段的提交，或范围跨越多个不同编号阶段，整个范围 Revert 都必须使用 `变动内容：`。
...
- 普通阶段任务提交：正文标题必须是 `阶段 N 变动内容：`。
- Merge：通常使用 `变动内容：`，因为 Merge 不属于普通阶段任务提交，其语义由合并主题和引入的逻辑变动表达。
- Revert：正文标题根据被撤销对象是否属于明确编号阶段决定，不由 Revert 本身的提交类型或仓库当前阶段决定。
...
- 提交前必须核对暂存 diff、提交标题、`type`、`scope`、开发阶段归属及对应的正文标题形态、逻辑正文和提交粒度。
...
- 必须逐提交审查标题、`type`、`scope`、正文、正文标题形态、粒度和依赖，不得只做格式替换。
...
历史重写只能在用户明确授权推送时发布，并且只允许带明确旧 SHA 的 `--force-with-lease=refs/heads/<分支>:<旧 SHA>`；禁止裸 `--force`，也禁止不带显式旧 SHA 的 `--force-with-lease`。推送前必须重新 `git fetch` 并确认远端仍停在该旧 SHA；一旦远端发生移动，立即停止强推、审查新增远端提交并重新生成重写计划。远端备份分支和备份 Tag 必须先于 `main` 推送成功，并保留到用户另行决定删除为止。不得自动创建 PR、删除备份引用或把普通推送授权解释为历史重写授权。若无法为全部提交重新签名、无法证明最终 tree 一致或逐提交验证失败，必须恢复原始历史并报告阻塞。
...
```text
docs(审计): 校正阶段五与阶段六门禁结论并归档计划演进

变动内容：
- 将阶段五与阶段六的门禁状态由已完成下调为未通过
- 归档实施计划的历次演进并保留被推翻结论的修正依据
- 同步用户向状态摘要与测试约定至审计事实
- 登记本轮未关闭的缺陷编号与后续裁决入口
```
...
错误原因：`开发阶段：阶段 N` 独立字段行已被废止，阶段提交只能使用单行正文标题 `阶段 4 变动内容：`。
...
错误原因：该能力明确属于阶段 4，正文标题必须写 `阶段 4 变动内容：`。
...
错误原因：被撤销对象属于阶段 4 的 Correctness 修复，Revert 必须在 `撤销对象：` 之后使用 `阶段 4 变动内容：`，不能因为“Merge 和 Revert 都不写阶段编号”而省略。
...
错误原因：被撤销对象是 Agent 规则文档调整，不属于任何编号开发阶段，不能因为“所有阶段任务提交都写阶段编号”而给 Revert 机械附加阶段正文标题。
...
3. 阶段任务使用唯一合法的 `阶段 N 变动内容：`，非阶段任务使用 `变动内容：` 且没有机械添加阶段编号；Merge 通常使用非阶段正文标题，Revert 已按被撤销对象是否属于编号阶段正确选择正文标题。
