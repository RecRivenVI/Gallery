...
- 当前仓库已有正式产品代码（`cmd/`、`internal/`、`pkg/`、`web/`）。阶段 0 契约骨架、Walking Skeleton、Architecture Proof 正确性切片、阶段 1「领域和数据所有权」、阶段 2「规则闭环」、阶段 3「扫描、任务和 Catalog」与阶段 4「查询和媒体」均已完成代码与合成 Correctness 实现；阶段 5「账户、安全和多客户端」已完成代码/合成安全收尾，并取得 Chrome/Edge 同机 Personal/LAN 主路径与当前工作站 Argon2id 补证；EV-48 又关闭规则递归深度崩溃及空格式、Windows 路径、Range、Cursor 的非规范输入缺口，EV-60 再以隔离 Chromium/真实 `galleryd` 覆盖 Personal Session/API Token/Share 和独立 loopback LAN 账户/Grant/Session 管理链，EV-86 继续关闭 Creator/Library 聚合封面按逐主体 deny、Token scope 与独立 `media.read` 裁剪的合成授权缺口，但正式 Security Gate 尚未通过。阶段 6 已实现 React/TypeScript Web/PWA 页面代码基线、OpenAPI 生成客户端、同源嵌入资产、静态壳 PWA，以及认证、浏览/媒体、Overlay、任务、规则、安全和维护页面；EV-54～EV-58 已把管理自举、publication-bound 画廊/媒体、CustomCover、规则发布首链、无损文本与 Schema 表单接入隔离 Chromium/真实 `galleryd` 持续门禁，EV-59 又增加 rollback、Package/Version/ParameterSet deprecate、共享参数 CAS/Impact 与 Binding/Job 执行快照不变量，EV-60 补齐首条安全管理写链与真实 WebSocket 断线后的 HTTP snapshot 恢复，EV-61 覆盖 control 备份、恢复干跑/待重启登记和 Catalog GC dry-run，EV-62 再覆盖 SourceRuleBinding 暂停/恢复、作品人工解绑/撤销及 retry-backoff Job 取消/同 ID 重试与 Attempt 历史，EV-64 进一步以同一隔离 AppDirs 的实际重启证明 control 恢复生效和 `restore.finalize` 审计，EV-65 把完整隔离链扩展到桌面 Firefox 并纳入 CI，EV-66 又关闭扫描/Watcher 与维护终态同步竞态，EV-67～EV-70 再补齐全部现有 primitive config 的 Schema 字段、当前草稿 Dry Run/Explain/Trace、按字段撤销及参数 Schema/tests/extensions 无损结构化编辑，EV-71 又补齐真实单帧 sequence gap 的 HTTP 全快照恢复，EV-72 再从双浏览器可见 UI 验证运行中 incremental Scan 取消、活动 Hash 子任务级联、Attempt/publication 收敛与 Source guard，EV-73 又以真实 `galleryd` 强杀和同 AppDirs 立即重启验证未来租约下的启动接管、recovered Attempt 与 UI 治理，EV-74 再覆盖 Binding issue 忽略/重开/解决、SourceWork split 决策/撤回、Work orphan 延长及 Media 解绑/显式撤销，EV-75 继续覆盖 SourceWork merge、已消费决策撤回冲突及完整 orphan decision/实体类型集合，EV-76 又修复问题重开的活动唯一性并覆盖普通三决定、真实生命周期、双标签页冲突与 51 条分页，EV-77 再补齐三种剩余结构 action 的实际消费和 Work/Creator/Media 孤儿重现身份语义，EV-91 又把 Job 历史改为逐资源授权后的新到旧 keyset 分页并在管理端建立连续加载/错误恢复，但正式 Web Gate 尚未通过。EV-92 首次把 legacy schema v3 的真实 Pixiv 规则经正式转换接入独立 AppDirs，在 370,712 文件/105,202 目录上完成 45 秒有界 `index`、最终 cancelled 与全树零写入 guard，并修复长 guard 期间 Watcher 抢跑的 testlab 编排缺陷；EV-93 随后让 discovery 的每个 WalkDir 回调及时检查 Scan context，关闭已确认的取消延迟根因，但尚待真实 Pixiv 复测。这些证据不是全量扫描、规则语义或性能 Gate。阶段 4 的 EV-35/EV-36 Correctness/Cursor 结论保持；EV-51 已把 500,000 合成 WorkProjection 的单并发搜索候选路径降到两位数毫秒 P95，EV-52 又把完整候选验证与 publication 指针切换拆分并以持久验证封印 fail-closed，EV-87 再以 catalog v20 持久 Creator/Source 窄候选建立 500,000 受限重选基线，EV-88 又把 Creator/Library 详情收窄为单 scope 并在 500,000 Work 下取得 0.55～1.00 ms P95，EV-89 再把用户端 Creator 改为授权 NaturalSortKey keyset 分页并以双浏览器真实后端链锁定 Source 范围；但兼容无参数全量入口、正式十来源完整关系、1%/10%/50% 变化 publication 多样本、并发与完整 Degradation 矩阵仍未完成，因此 Reference Performance Gate 仍未通过，Gank/Pawchive 有界验证也仍未完成。
...
- [EV-89](Documents/证据/验证记录.md)（2026-07-29）为 `GET /api/v1/creators` 增加兼容分页浏览模式：无查询参数仍返回全部 CanonicalCreator，浏览参数则在授权 active Source 与有效合并根上按 NaturalSortKey v2/keyset 分页，cursor 绑定授权与查询指纹；Source 作者页、等价组封面和后续作品过滤保持同一平台范围。control v23 增加/原子回填 Creator 排序键；100,000 Creator 无合并 P95 为 0.621 ms，一个合并图 P90 43.6 ms。Chromium/Firefox mock 与各 21 项真实后端完整链均覆盖该入口；兼容全量响应、并发/冷缓存/超大合并图、API Freeze 与正式 Web/Performance Gate 仍未完成。
...
- [EV-91](Documents/证据/验证记录.md)（2026-07-29）把 Job 历史从最旧优先的全量读取/N+1 改为 control v24 索引支持的新到旧 keyset 分页；严格 cursor 绑定状态、limit 与授权指纹，但只作为边界，所有候选仍在响应 limit 前逐 Job 重新授权。管理端以 50 项连续加载，续页失败保留已加载页面，状态切换丢弃旧 cursor；Chromium/Firefox 新增真实后端定向用例和低资源 WSL2 race 均通过。该证据没有重跑既有 21 项完整浏览器链，不代表超大历史性能、API Freeze、Web Gate 或 RC 已完成。
...
- 尚未完成：阶段 5 真实物理 LAN 多设备、目标低端设备 Argon2id 和真实恶意资源门禁；阶段 6 真实移动设备/触控、人工屏幕阅读器、任意规则的完整可视化构建、真实存储上的取消与崩溃恢复响应、完整弱网抖动矩阵与完整可访问性 E2E；以及 Pixiv/真实 HDD 全量扫描与哈希、SMB/NAS、真实 FileID/`dev+inode`、正式 Reference/Degradation Performance Gate、ranking/total/租约等 PRE_FREEZE 数值、AND/OR canonical 化和平台发行。
