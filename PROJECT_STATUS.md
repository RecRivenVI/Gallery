...
| 阶段 3：扫描、任务与目录库 | ✅（代码与模拟数据层面） | 🟡（真实大盘抽样与三平台有界链通过，全量未完成） | EV-92～EV-94 在真实 Pixiv 上完成 discovery 取消/恢复与零写入；EV-108/111 完成 Gank/Pawchive 确认链；EV-112 在 Pawchive 上观察真实 `hash/running` 后取消，4 ms 内观察父子终态且全树零变化 | Pixiv/真实盘全量扫描与哈希、HDD/SMB/NAS 取消、publishing/崩溃恢复、正式性能门禁和网络共享盘尚未完成 | 继续其它存储与异常切片，同时收口阶段 4 正式压力门禁 |
...
| 阶段 5：账户、安全与多客户端 | 🟠（代码与合成安全收尾已实现；恶意输入缺陷已收口） | 🟡（Personal、同机 LAN、真实进程树/ffprobe、生产 ToolDiscovery 与 Windows OS 资源硬限额补证，正式 Gate 未通过） | EV-37/EV-38/EV-44/EV-48 的安全收口之外，EV-60 已把 Session、API Token、Share、allow/deny Grant、账户停用恢复和精确 Session 吊销接入隔离真实浏览器；EV-86 又让 Creator/Library 聚合封面遵守 deny、Token scope 与独立 `media.read`；EV-114/115 覆盖真实 OS 进程树与 ffprobe，EV-126 接入不搜索 PATH、精确版本/SHA-256 pin 的生产 ToolDiscovery，EV-127 再以 Windows Job Object 对整棵工具进程树施加累计 CPU 与聚合提交内存硬限制 | 真实 LAN 多设备、目标低端设备 Argon2id、恶意媒体语料库及非 Windows 等价资源门禁仍未完成；同机 loopback 不能替代正式 Security Gate | 完成外部设备、恶意资源与其余平台门禁 |
| 阶段 6：Web/PWA 界面 | 🟠（前端双入口、设计重构与首批真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实 `galleryd` E2E 已建立，正式 Gate 未通过） | 共享设计系统、媒体优先画廊端与紧凑管理端；EV-54～EV-91 覆盖主要业务/治理/恢复与弱网切片，EV-102 部分采纳共享动效基座，EV-113～EV-118 建立全路由/关键状态可访问性组合并把双入口配对与管理安全/维护状态接入真实后端 | 浏览器业务门禁仍使用合成 Source；EV-92 只补到独立 testlab 的真实 Pixiv 有界预检。真实存储浏览器链、其余弱网矩阵、真实缩放/系统高对比、触摸设备与屏幕阅读器未验证；画廊端无 DOM 虚拟化 | 扩大真实后端业务与可访问性 E2E，不进入桌面壳 |
| 阶段 7：平台适配与正式发行 | 🟠（Windows 便携、恢复/回滚及首条真实历史升级边界已建立） | 🟡（本地制品、schema 23→24/反向拒绝、包内与进程级恢复失败门禁通过，正式 Gate 未通过） | EV-103～105 完成便携、同源切换、正常/损坏备份；EV-106 证明真实 schema 23→24、旧程序拒绝新 migration 与当前程序复启；EV-107/110 覆盖轮换/落位拒绝；EV-109/119 关闭落位后连续性缺口；EV-121～123 关闭当前库缺失、安全收尾和状态文件失败；EV-124 关闭便携同进程双 Rename 失败/恢复；EV-125 再关闭 finalize 持久阶段后的真实进程强杀/续接 | Authenticode、安装/更新、CredentialStore、更多支持版本跨度、磁盘满/ACL/低权限、其它恢复窗口强杀/真实断电、平台矩阵及桌面壳均未完成 | 先完成签名、其余失败回滚和 Windows RC 门禁 |
...
| 威胁模型测试（路径攻击、恶意元数据、恶意媒体文件、限流） | 需要 | 认证爆破、Host/Origin/CSRF/Content-Type、WS 防滥用，以及路径/metadata/媒体正文/恢复输入均进入正式合成测试；真实测试二进制覆盖进程树强杀，显式真实 ffprobe 覆盖版本输出基线、输出溢出、超时与截断 MP4；生产 ToolDiscovery 已固定显式路径、版本和 SHA-256；Windows Job Object 已限制整棵工具树累计 CPU 与提交内存 | `internal/auth/security_test.go`、`internal/transport/httpapi/security_api_test.go`、`internal/contract/realtime/envelope_test.go`、`internal/toolrunner/{integration_test.go,actual_ffprobe_test.go}`、`internal/platform/{process,tooldiscovery}`、`tools/testlab/stages/stage5/security` | ✅（合成攻击矩阵 + 真实 OS 进程树/ffprobe/生产发现/Windows 资源硬限额切片） | 合成安全回归 + 高重复/race + Windows/WSL2 真实进程树 + Windows 显式 ffprobe/Job Object + 隔离生产启动 | EV-09、EV-37、EV-114～EV-115、EV-126～EV-127 | 🟡 | 单个合成截断容器不是恶意语料库；非 Windows 等价资源门禁仍缺，也不代表真实 LAN；整体 Security Gate 未通过 |
...
| 功能项 | 初始计划 | 当前计划 | 事实代码证据 | 功能状态 | 测试/门禁项目 | 文档证据 | 测试状态 | 局限或缺口 |
|---|---|---|---|---|---|---|---|---|
| Windows 正式支持（真实运行、升级、崩溃恢复、签名） | 需要 | 增加无壳便携测试制品、恢复/回滚及首条真实历史升级边界作为 RC 前基线 | `pkg/galleryversion`、Windows 发行脚本、`tools/testlab/cmd/{portable,historical}-upgrade`、Windows CI | 🟠 | 精确干净提交构建、版本/摘要/SBOM/签名事实、同 AppDirs 强杀重启、正常/损坏备份、真实 schema 23→24/反向拒绝、Windows 轮换/落位、包内与真实便携进程双 Rename、当前库缺失、安全收尾、状态文件失败恢复及 finalize 持久阶段强杀续接 | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-125 | 🟡（本地制品、单条历史升级、包内与多条恢复进程门禁通过） | 仍未完成正式签名、安装/更新、更多支持版本跨度、磁盘满/ACL/低权限、其它恢复窗口强杀/真实断电、多用户及完整 Windows 平台门禁 |
...
| 安装包、签名、SBOM（软件物料清单）、依赖安全检查 | 需要 | 先建立 Windows x64 便携 ZIP、恢复/回滚与真实历史迁移门禁，再完成安装与签名 | EV-103 已生成三个 CycloneDX SBOM、清单、包内/外 SHA-256，构建器支持 Authenticode fail-closed；EV-104～107 覆盖同源切换、正常/损坏备份、真实 schema 23→24/反向拒绝和首次轮换失败；EV-109/110/119/121～125 增加落位后及 pre-placement 连续性 fail-closed、真实候选落位拒绝、包内/进程级双失败、安全收尾续接、状态文件失败恢复与 finalize 持久阶段强杀续接；CI 继续执行依赖审计/漏洞扫描 | 🟠（部分） | 本地正向/负向制品 smoke、恢复/回滚、单条真实历史升级边界、真实 Windows 落位安全回滚、包内与便携进程双 sharing-violation、当前库缺失/stale incoming、安全收尾、状态文件失败恢复及精确强杀窗口；Windows CI 与手工来源证明 workflow | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-125、`ci.yml`、`windows-portable.yml` | 🟡 | 当前精确包仍为 `unsigned`；无安装器、证书/时间戳、自动更新、完整支持版本跨度或签名后平台证据，不能作为 RC |
...
|---|---|---|---|---|---|---|
| 全部 Go 测试（786 个顶层 `Test*`/`Benchmark*`/`Example*` 函数，覆盖 66 个目录/包） | 贯穿各阶段 | `cmd`/`internal`/`pkg`/`tools` 共 182 个 `*_test.go` 文件 | `scripts/Check.ps1` 每次运行 | ✅（在 CI 上持续运行并要求全部通过） | Windows + Ubuntu（GitHub Actions） | 绝大多数是模拟/合成数据；部分显式门禁使用真实 OS/工具/Source，局限分别登记；Argon2id benchmark 需手动 `-bench`，CI 从不执行 |
...
| Windows/Linux/macOS/Docker 平台支持 | 阶段 7 | Windows 增加本地便携测试包、恢复/回滚、真实 schema 23→24/反向拒绝、轮换/候选落位失败、包内及便携进程双失败、当前库缺失/stale incoming、安全收尾、状态文件失败及 finalize 持久阶段强杀进程门禁；CI 仍只有 Windows + Ubuntu | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-125、ADR-007 四级支持成熟度表 | 🟡（Windows 增加制品/恢复/升级/多条真实失败门禁，Linux 仍停在 CI 层级）/ ⏳（macOS/Docker） | 当前工作站 Windows x64 + GitHub Actions 配置 | 未达到"发行候选"或"正式支持"级别；本地无签名包，Windows 本机结果与 CI Linux 都不能外推为正式平台支持 |
| 安全（认证、授权、Web 边界、路径穿越、恶意元数据/媒体、限流） | 阶段 5 | 正式生产包覆盖账户/Token/Grant/Session/WS/Web 与合成攻击；Work 查询逐成员授权；真实 Chrome/Edge 已覆盖 Personal/LAN 主路径和吊销；外部工具输出/超时已覆盖真实 OS 进程树及显式真实 ffprobe，生产 ToolDiscovery 已固定显式路径、精确版本和 SHA-256，Windows Job Object 已加入进程树 CPU/内存硬限制 | EV-09、EV-37、EV-38、EV-44、EV-60、EV-114～EV-115、EV-126～EV-127 | 🟡 | Windows 合成与浏览器；Windows 真实进程树/ffprobe/Job Object/隔离生产启动；WSL race；隔离 Personal/LAN Chromium 安全管理链 | 真实物理 LAN 多设备、目标低端设备、恶意媒体语料库与非 Windows 等价资源门禁未完成，整体 Gate 未通过 |
| Web/PWA 界面测试 | 阶段 6 | `web/src/**/*.test.ts(x)`、`web/scripts/check-audit.test.mjs`、`web/e2e`、`internal/webapp/*_test.go` 与 `tools/testlab/cmd/web-e2e` | EV-38～EV-40、EV-42、EV-44、EV-54～EV-91、EV-102、EV-113、EV-116～EV-118 | 🟡 | Vitest 15 个文件 212 项；Chromium/Firefox mock smoke 26/26；19 条路由在桌面/窄屏各 38 个状态通过 axe，并在 320px 模拟强制颜色/文本间距下覆盖路由及五个 mock 关键交互；EV-118 双浏览器定向各 1/1 覆盖真实双入口配对、Token 和维护状态。当前完整真实运行器含 23 项；本轮生产修复后的完整 23 项未全量复跑 | 自动路由基线、作品身份交接、窄屏焦点及主要真实业务链已覆盖；真实移动/触控、人工屏幕阅读器、实际缩放/物理高对比、其余交互状态和弱网矩阵仍未完成 |
...
| 发布/签名/SBOM | 阶段 7 | Windows 便携包、SBOM、清单、摘要、可选 Authenticode、恢复/回滚及真实历史升级边界已进入脚本与 CI；EV-109/110/119/121～125 补齐落位后/pre-placement 连续性、真实候选落位安全回滚、包内与便携进程双失败、安全收尾续接、状态文件失败恢复和 finalize 持久阶段强杀门禁 | EV-103～EV-107、EV-109、EV-110、EV-119、EV-121～EV-125、Windows 发行脚本与 workflow | 🟡（未签名测试基线） | 本地精确干净 ZIP 正向/负向、正常/损坏备份、schema 23→24/反向拒绝、轮换/候选落位失败、包内/便携进程双失败、当前库缺失/stale incoming、安全收尾、状态文件失败及精确强杀窗口的进程 fail-closed/重试；远端 workflow 待当前提交触发 | 正式证书/时间戳、安装器、更新、更多支持版本跨度、磁盘满/ACL/低权限、其它恢复窗口强杀/真实断电及 RC 仍不存在 |
...
| 已修正的外部工具测试漂移（EV-114/115/126/127） | 已新增 Argon2id benchmark，但只在当前工作站取得 EV-38 数据且 CI 从不执行；全仓库仍无正式 Fuzz。此处旧文称 `internal/toolrunner` 的「有界输出/超时/强杀」测试从未真正触及上限，但提交 `2af146a` 早已让 1,024-byte 输出上限和 3 秒超时驱动真实父子孙进程树终止，EV-114 重新复验；EV-115 又用显式真实 ffprobe 覆盖输出、超时与截断容器，EV-126 接入生产 ToolDiscovery 与精确版本/摘要 allowlist，EV-127 再加入 Windows 进程树 CPU/内存硬限制 | 已关闭“上限未被触及”“完全没有真实工具补证”“生产 Resolver 为空”和“Windows 无 OS 资源硬限制”的缺口；恶意媒体语料库、非 Windows 等价门禁、目标低端设备 Argon2id 与 Fuzz 仍不能算完成 |
...
| 安全风险 | 阶段 5 账户/凭据/授权、匿名 Share、全资源矩阵、恶意输入与 WS 防滥用代码及合成测试已落地；EV-44 已关闭 Work 聚合查询逐成员授权缺口，EV-46 已关闭 `SEC-3`，EV-126/127 已接入生产 ToolDiscovery 与 Windows 进程树资源硬限制；真实 LAN 多设备/浏览器、目标设备 Argon2id、恶意媒体语料库和非 Windows 等价门禁仍缺，不能描述为完整 Security Gate 通过 |
| 产品/UI 缺口 | Web/PWA 代码基线已存在，EV-39 的实时通道与写入口阻断已由 EV-40 关闭；EV-54～EV-91 覆盖主要真实后端业务与治理持续链，EV-113/116 又把当前 19 条路由的桌面/窄屏稳定状态及模拟强制颜色、文本间距、400% 等效重排纳入双浏览器 axe，EV-117 再补五个关键交互状态，EV-118 把双入口配对及管理安全/维护关键状态接入真实后端并修复换壳瞬态语义。EV-92 的真实 Pixiv 证据属于独立 testlab，不是浏览器业务链；真实移动设备/触控、人工屏幕阅读器、实际浏览器缩放、物理操作系统高对比、其余交互状态组合与正式可用性 Gate 均未完成 |
...
| 发行缺口 | EV-103 已有未签名 Windows 便携 ZIP、三个 SBOM、摘要与 smoke；EV-104～107 再覆盖正常/损坏备份、真实 schema 23→24/反向拒绝及首次轮换失败；EV-109/110/119 关闭落位后空库风险并证明安全回滚/包内双失败，EV-121～123 关闭当前库缺失、安全收尾与状态文件失败，EV-124 关闭便携同进程双 Rename，EV-125 再关闭 finalize 持久阶段后的真实强杀续接。仍没有安装包、正式代码签名/时间戳、自动更新、CredentialStore、完整支持版本跨度、磁盘满/ACL/低权限、其它恢复窗口强杀/真实断电和平台支持证据 |
...
| 3 | 阶段 6 | EV-92～EV-94 已完成真实 Pixiv Source 只读有界预检，EV-112 已关闭 Pawchive/本地 SSD 活动 Hash 取消，EV-113/116 已建立当前全路由组合 axe，EV-117 已补五个关键交互状态，EV-118 又把其中的双入口配对与管理安全/维护状态接入真实后端。下一步继续 HDD/SMB/NAS、publishing/崩溃恢复、其余弱网/交互状态、人工辅助技术、真实缩放/系统高对比与真实设备 | Pixiv 全量扫描保持低优先级，不能替代业务闭环和发布可用性门禁 |
| 4 | 阶段 7 | 在 EV-103～107、EV-109/110/119/121～125 的便携、恢复/回滚与真实 schema 23→24 基线上完成 Authenticode、支持版本跨度、磁盘满/ACL/低权限、其它恢复窗口强杀/真实断电及 Windows RC；随后推进 Linux 原生、macOS、Docker、SMB/NAS 和安装发行 | 当前关闭制品编排、正常/损坏备份、一条相邻历史边界、轮换/落位失败、包内与便携进程双失败、当前库缺失、安全收尾续接、两类状态文件失败及一个精确强杀窗口；签名、其余异常和平台支持仍是正式发行阻断项 |
