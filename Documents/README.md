...
2026-07-28 的 [EV-65](证据/验证记录.md) 已把现有 Personal 11/11、LAN 1/1、同 AppDirs 恢复重启和只读 Source guard 的隔离真实后端链从 Chromium 扩展到桌面 Firefox，并让 Chromium/Firefox smoke 与完整业务链进入 CI；[EV-66](证据/验证记录.md) 又关闭显式扫描/Watcher 状态脱节、扫描期间事件丢失与维护 Job 终态不刷新的竞态。阶段 6 Web Gate 仍因真实移动设备/屏幕阅读器、完整治理、长任务恢复与正式可用性门禁缺口而未通过。
