...
| 阶段 6：Web/PWA 界面 | 🟠（页面代码基线与首批真实业务链路已实现） | 🟡（隔离 Chromium/Firefox 真实后端 E2E 已建立；正式 Gate 未通过） | 同源 Web/PWA 覆盖浏览与管理页面；空实例自举、publication-bound 画廊/媒体、CustomCover、规则生命周期/ParameterSet/Schema 表单、当前草稿 Dry Run/Explain/Trace、按字段撤销、参数 Schema/tests/extensions 无损结构化编辑、安全管理、维护、规则绑定状态、作品/媒体人工解绑与撤销、普通 Binding issue 三决定/生命周期/分页、全部五种 SourceWork 结构 action、全部 orphan decision/实体类型及重现身份语义、已消费决策冲突、retry-backoff Job、运行中 Scan/Hash 级联取消、进程强杀启动接管、control 恢复实际重启、WS 断线与单帧 sequence gap 的 snapshot 恢复已有双浏览器持续 E2E；扫描/Watcher 与维护终态同步竞态已关闭 | 真实存储取消与崩溃恢复响应、完整弱网抖动矩阵、真实移动设备/屏幕阅读器及窄屏焦点陷阱未完成 | 扩大真实业务与可访问性门禁，不进入桌面壳 |
